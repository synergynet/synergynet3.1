...
public class Poses
{
	...
	/** The gest seqs. */
	private PoseSequences gestSeqs; // stores gesture sequences for each user,
									// and looks for more complex gestures
									// (actually only right hand waving at
									// present)

	/** The tracker panel. */
	private TrackerPanel trackerPanel; // object that is notified of an gesture
										// start/stop by calling its pose()
										// method
	 ...
	 *            the depth gen
	 */
	public void checkGests(int kinectID, DepthGenerator depthGen)
	{ // decide which gestures have just started or just finished, and notify
		// the watcher.
		 ...
		 * will change if the user moves closer or further away. This overhead
		 * would disappear if skeletons were stored using real-world coordinates
		 * instead of screen-based values.
		 */
	 ...
	 * @return the float
	 */
	private float distApartXY(Point3D p1, Point3D p2)
	{ // the distance between the two points
	 ...
	 * @return the float
	 */
	private float distApartXZ(Point3D p1, Point3D p2)
	{ // the distance between the two points
	 ...
	 *            the gesture
	 */
	private void leanBack(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's head leaning back relative to their torso?
	 ...
	 *            the gesture
	 */
	private void leanFwd(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's head forward of their torso?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leanLeft(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's head to the left of their left hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leanRight(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's head to the right of their right hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leftElbowUp(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	 ...
	 *            the gesture
	 */
	private void leftHandDown(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	 ...
	 *            the gesture
	 */
	private void leftHandInHigh(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's left hand inside (right) of their left elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leftHandInLow(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's left hand inside (right) of their left elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void leftHandOutHigh(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's left hand out to the left of the their left elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leftHandOutLow(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's left hand out to the left of the their left elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void leftHandTouchHip(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's left hand touching their left hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void leftHandUp(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	 ...
	 *            the gesture
	 */
	private void rightElbowUp(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand at head level or above?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void rightHandDown(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	 ...
	 *            the gesture
	 */
	private void rightHandInHigh(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand inside (left) of their right elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void rightHandInLow(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand inside (left) of their right elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void rightHandOutHigh(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand out to the right of the their right elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void rightHandOutLow(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand out to the right of the their right elbow?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void rightHandTouchHip(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand touching their right hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void rightHandUp(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // is the user's right hand at head level or above?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void turnLeft(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // has the user's right hip turned forward to be in front of their left
		// hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void turnRight(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // has the user's left hip turned forward to be in front of their right
		// hip?
		if (!poseActivity.containsKey(gesture))
		{
			return;
	 ...
	 *            the gesture
	 */
	private void twoHandsApart(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // are the user's hand apart?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void twoHandsNear(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // are the user's hand close together?
		if (!poseActivity.containsKey(gesture))
		{
			return;
		}
	 ...
	 *            the gesture
	 */
	private void twoHandsNearTorso(int kinectID, HashMap<SkeletonJoint, SkeletonJointPosition> skel, PoseName gesture)
	{ // are the user's hand close together?
		if (!poseActivity.containsKey(gesture))
		{
			return;
