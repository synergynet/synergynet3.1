...
public class Skeletons {
	...
	class CalibrationCompleteObserver implements
		...
		public void update(
			...
			if (trackerPanel.getTeacherControlPanel().isTeacher(userID)) {
				UserTracker.writeToAnnouncementBox("Calibration status: "
						+ args.getStatus() + " for person " + userID);
				try {
					if (args.getStatus() == CalibrationProgressStatus.OK) {
						// calibration succeeded; move to skeleton tracking
						UserTracker
								.writeToAnnouncementBox("Starting tracking person "
										+ userID);
	...
	class LostUserObserver implements IObserver<UserEventArgs> {
		...
		public void update(IObservable<UserEventArgs> observable,
			...
			UserTracker
					.writeToAnnouncementBox("Lost track of person " + userID);
	...
	class PoseDetectedObserver implements IObserver<PoseDetectionEventArgs> {
		...
		public void update(IObservable<PoseDetectionEventArgs> observable,
			...
			int userID = args.getUser();
			UserTracker.writeToAnnouncementBox(args.getPose()
					+ " pose detected for person " + userID);
			try {
	 ...
	 *
	 * @param calibPoseName the new calib pose name
	 */
	 ...
	 * Sets the gest seqs.
	 *
	 * @param gestSeqs the new gest seqs
	 */
	 ...
	 *
	 * @param poseDetectionCap the new pose detection cap
	 */
	 ...
	 * Sets the skel cap.
	 *
	 * @param skelCap the new skel cap
	 */
	 ...
	 *
	 * @param skelsGests the new skels gests
	 */
	 ...
	 * Configure.
	 */
	private void configure() {// create pose and skeleton detection capabilities
		...
		try {
			...
			skelCap.getCalibrationCompleteEvent().addObserver(
					new CalibrationCompleteObserver()); // for when skeleton
														// calibration is
														// completed, and
														// tracking starts
		} catch (Exception e) {
	 ...
	 * @throws StatusException the status exception
	 */
	private void drawUserStatus(Graphics2D g2d, int userID)
			throws StatusException { // draw user ID and status on the skeleton
	 ...
	 * @param userID the user id
	 */
	private void setLimbColor(Graphics2D g2d, int userID) { // use the
															// 'opposite' of the
															// user ID colour
															// for the limbs, so
															// they stand out
	 ...
	 * @param userID the user id
	 */
	private void updateJoints(int userID) { // update all the joints for this
											// userID in userSkels
		HashMap<SkeletonJoint, SkeletonJointPosition> skel = userSkels
				.get(userID);
	 ...
	 * @param skel the skel
	 */
	private void updateSkeletonPos(int userID,
			HashMap<SkeletonJoint, SkeletonJointPosition> skel) {
		...
		try {
			if ((p1 != null) && (p2 != null)) {
				...
				TrackerNetworking.setBothUserHandLocations(userID, p1.getX(),
						p1.getY(), p1.getZ(), p2.getX(), p2.getY(), p2.getZ());
			} else if (p1 != null) {
				p1 = depthGen.convertProjectiveToRealWorld(p1);
				TrackerNetworking.setSingleUserHandLocation(userID, p1.getX(),
						p1.getY(), p1.getZ());
			} else if (p2 != null) {
				p2 = depthGen.convertProjectiveToRealWorld(p2);
				TrackerNetworking.setSingleUserHandLocation(userID, p2.getX(),
						p2.getY(), p2.getZ());
			}
		} catch (StatusException e) {
			e.printStackTrace();
	 ...
	 * @throws StatusException the status exception
	 */
	private void updateUserPos(int userID) throws StatusException { // draw user
																	// ID and
																	// status on
																	// the
																	// skeleton
		...
		TrackerNetworking.setUserBodyLocation(userID, massCenter.getX(),
				massCenter.getZ());
	}
