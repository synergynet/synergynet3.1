...
public class MultiplexerSync {
	...
	private DistributedPropertyChangedAction<String> teacherStatusChanged = new DistributedPropertyChangedAction<String>() {
		...
		public void distributedPropertyDidChange(Member member,
			...
			String userID = CombinedUserEntity.generateUserIDString(
					TrackerUtils.getUserIdFromMessage(newValue),
					TrackerUtils.getTableIdFromTeacherStatusMessage(newValue));
						...
						if (user.isTeacher() != isTeacher) {
							if (isTeacher) {
								Multiplexer.writeToAnnouncementBox("Entity "
										+ user.getUniqueID()
										+ " has been set to teacher");
							} else {
								Multiplexer.writeToAnnouncementBox("Entity "
										+ user.getUniqueID()
										+ " has been set to student");
							}
							user.setTeacher(isTeacher);
							TrackerUpdater
									.sendTeacherStatusUpdateToTrackers(user);
						}
						break;
					}
				}
	...
	private DistributedPropertyChangedAction<UserLocations> userLocations = new DistributedPropertyChangedAction<UserLocations>() {
		...
		public void distributedPropertyDidChange(Member m,
				UserLocations oldValue, UserLocations newValue) {
			...
			synchronized (Multiplexer.users) {
				...
				for (UserLocation userLoc : newValue.getUserLocations()) {
					...
					String userID = CombinedUserEntity.generateUserIDString(
							userLoc.getID(), source);
					boolean newID = true;
					for (CombinedUserEntity user : Multiplexer.users) {
						if (user.getUserIDs().contains(userID)) {
							user.setUserLocation(MultiplexerUtils
									.mergeUserLocations(userLoc,
					...
					if (newID) {
						CombinedUserEntity user = new CombinedUserEntity(
								userID,
								userLoc,
								MultiplexerUtils
										.getLowestAvailableUniqueCombinedUserID());
						Multiplexer.users.add(user);
						Multiplexer.writeToAnnouncementBox("Discovered Entity "
								+ user.getUniqueID() + " (Person "
								+ userLoc.getID() + " from " + source + ")");
				...
				if (existingIds.containsKey(source)) {
					for (int i : existingIds.get(source)) {
						if (!newIDs.contains(i)) {
							String userID = CombinedUserEntity
									.generateUserIDString(i, source);
					...
					for (CombinedUserEntity user : Multiplexer.users) {
						if (user.getUserIDs().contains(userID)) {
							...
							if (user.getUserIDs().size() == 0) {
								toRemove.add(user);
								Multiplexer
										.writeToAnnouncementBox("Lost track of Entity "
												+ user.getUniqueID());
							}
							break;
						}
	 ...
	 */
	public void stop() {
		c.getTeacherStatusControlVariable().unregisterChangeListener(
				teacherStatusChanged);
		c.getTrackedUserLocationsControlVariable().unregisterChangeListener(
				userLocations);
	}
	...
	private void addSync() {
		c.getTeacherStatusControlVariable().registerChangeListener(
				teacherStatusChanged);
		c.getTrackedUserLocationsControlVariable().registerChangeListener(
				userLocations);
	}
