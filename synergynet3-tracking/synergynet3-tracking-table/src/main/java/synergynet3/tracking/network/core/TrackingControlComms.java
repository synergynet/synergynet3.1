...
public class TrackingControlComms {
	...
	private static final Logger log = Logger
			.getLogger(TrackingControlComms.class.getName());
	 ...
	 *
	 * @param trackingTableIdentity the tracking table identity
	 */
	public void announceDeSelection(String trackingTableIdentity) {
		for (String device : getTrackersList()) {
			try {
				...
				std.getTableDeselectedControlVariable().setValue(
	...
	public void announceSelection(String trackingTableIdentity) {
		for (String device : getTrackersList()) {
			try {
				TrackingDeviceControl std = getDeviceForName(device);
				std.getTableSelectedControlVariable().setValue(
						trackingTableIdentity);
			} catch (Exception ex) {
			}
		}
	...
	public void broadcastUserLocations(UserLocations userLocations) {
		for (String device : getMultiplexersList()) {
			try {
				TrackingDeviceControl std = getDeviceForName(device);
				std.getTrackedUserLocationsControlVariable().setValue(
						userLocations);
			} catch (Exception ex) {
			}
		}
	...
	public TrackingDeviceControl getDeviceForName(String device) {
		try {
			TrackingDeviceControl std = (TrackingDeviceControl) SynergyNetCluster
	...
	public List<String> getMultiplexersList() {
		return SynergyNetCluster.get().getPresenceManager()
				.getDeviceNamesOnline("multiplexers");
	}
	...
	public int getNumberOfTrackersOnline() {
		return SynergyNetCluster.get().getPresenceManager()
				.getDeviceNamesOnline("trackers").size();
	}
	 ...
	 *
	 * @param message the new all tables selected mode enabled
	 */
	public void setAllTablesSelectedModeEnabled(PerformActionMessage message) {
		for (String device : getTablesList()) {
			try {
				TrackingDeviceControl std = getDeviceForName(device);
				std.getAllTablesSelectedModeEnabledControlVariable().setValue(
						message);
			} catch (Exception ex) {
			}
		}
	 ...
	 *
	 * @param message the new individual table select mode enabled
	 */
	public void setIndividualTableSelectModeEnabled(PerformActionMessage message) {
		for (String device : getTablesList()) {
			try {
				TrackingDeviceControl std = getDeviceForName(device);
				std.getIndividualTableSelectModeEnabledControlVariable()
						.setValue(message);
			} catch (Exception ex) {
			}
		}
	 ...
	 *
	 * @param message the new table selectd mode disabled
	 */
	public void setTableSelectdModeDisabled(PerformActionMessage message) {
		for (String device : getTablesList()) {
			try {
				TrackingDeviceControl std = getDeviceForName(device);
				std.getTableSelectedModeDisabledControlVariable().setValue(
						message);
			} catch (Exception ex) {
			}
		}
