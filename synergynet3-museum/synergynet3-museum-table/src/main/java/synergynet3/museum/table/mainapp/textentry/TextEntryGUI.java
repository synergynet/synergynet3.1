...
public class TextEntryGUI implements KeyboardOutput {
	 ...
	 */
	public TextEntryGUI(IStage stage, File folder, String id, String prompt)
			throws ContentTypeNotBoundException {
		this.prompt = prompt;
		this.folder = folder;
		this.id = id;
		container = stage.getContentFactory().create(IContainer.class,
				"textentryContainer", UUID.randomUUID());
		...
		textLabel.setText(MuseumAppPreferences.getTextInputInstructions(),
				stage);
		textLabel.setRelativeScale(TEXT_SCALE);
		textLabel.setRelativeLocation(new Vector2f(0, (HEIGHT / 2)
				- ((ITEM_HEIGHT / 2) * TEXT_SCALE) - (GAP / 2)));
		textLabel.setMovable(false);

		textScrollbox = new TextScrollBox(stage, WIDTH, ITEM_HEIGHT, GAP,
				TEXT_SCALE);
		textScrollbox.getTextLabel().setRelativeLocation(
				new Vector2f(0, (HEIGHT / 2) - (ITEM_HEIGHT * TEXT_SCALE) - GAP
						- ((ITEM_HEIGHT / 2) * TEXT_SCALE)));

		ISimpleKeyboard keyboard = stage.getContentFactory().create(
				ISimpleKeyboard.class, "keyboard", UUID.randomUUID());
		keyboard.setColours(INVIS, MuseumAppPreferences
				.getTextInputKeyboardButtonBackgroundColour(),
				MuseumAppPreferences.getTextInputKeyboardButtonBorderColour(),
				INVIS, MuseumAppPreferences
						.getTextInputKeyboardButtonFontColour());
		keyboard.setMovable(false);
		keyboard.generateKeys(stage, this);
		float keyboardScale = (HEIGHT - ((ITEM_HEIGHT * TEXT_SCALE) * 3) - (GAP * 1.5f))
				/ keyboard.getHeight();
		keyboard.setRelativeScale(keyboardScale);
		keyboard.setRelativeLocation(new Vector2f(0, (HEIGHT / 2)
				- ((ITEM_HEIGHT * 2) * TEXT_SCALE) - GAP
				- ((keyboard.getHeight() / 2) * keyboardScale)));

		IButtonbox okButton = stage.getContentFactory().create(
				IButtonbox.class, "enabledOkButton", UUID.randomUUID());
		okButton.setText("Submit", MuseumAppPreferences
				.getTextInputKeyboardButtonBackgroundColour(),
				MuseumAppPreferences.getTextInputKeyboardButtonBorderColour(),
		...
		okButton.setRelativeLocation(new Vector2f((-okButton.getWidth() / 2)
				- (GAP / 2), (-HEIGHT / 2) + (okButton.getHeight() / 2)
				+ (GAP / 2)));
		...
		cancelButton.setText("Cancel", MuseumAppPreferences
				.getTextInputKeyboardButtonBackgroundColour(),
				MuseumAppPreferences.getTextInputKeyboardButtonBorderColour(),
				MuseumAppPreferences.getTextInputKeyboardButtonFontColour(),
				BUTTON_WIDTH, ITEM_HEIGHT / 2, stage);
		cancelButton.setRelativeLocation(new Vector2f(
				(cancelButton.getWidth() / 2) + (GAP / 2), (-HEIGHT / 2)
	...
	private void saveText() {
		outputTextToFile(folder.getAbsolutePath() + File.separator + id
				+ ".txt");
		close();
	}
