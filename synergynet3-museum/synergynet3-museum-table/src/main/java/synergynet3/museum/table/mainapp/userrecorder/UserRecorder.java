...
public class UserRecorder implements IAudioItem {
	 ...
	 */
	public UserRecorder(String prompt, IStage stage, File folder,
			UserRecordingPromptLabel userRecordingPromptLabel,
			ITextbox entityItem, EntityManager entityManager)
		...
		audioController.setMaxRecordingTime(MuseumAppPreferences
				.getMaxRecordingTime());
		...
		enabledRecordStartButton = new RecorderButton(stage,
				"enabledRecordStartButton", "Start Recording", true);
		enabledRecordStartButton.getButtonContainer().setRelativeLocation(
				new Vector2f((-BUTTON_WIDTH / 2) - (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		...
		enabledRecordStartButton.getListener().getMultiTouchDispatcher()
		...
		disabledRecordStartButton = new RecorderButton(stage,
				"disabledRecordStartButton", "Start Recording", false);
		disabledRecordStartButton.getButtonContainer().setRelativeLocation(
				new Vector2f((-BUTTON_WIDTH / 2) - (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		disabledRecordStartButton.getButtonContainer().setVisible(false);

		enabledRecordStopButton = new RecorderButton(stage,
				"enabledRecordStopButton", "Stop Recording", true);
		enabledRecordStopButton.getButtonContainer().setRelativeLocation(
				new Vector2f((BUTTON_WIDTH / 2) + (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		...
		enabledRecordStopButton.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorPressed(MultiTouchCursorEvent event) {
		...
		disabledRecordStopButton = new RecorderButton(stage,
				"disabledRecordStopButton", "Stop Recording", false);
		disabledRecordStopButton.getButtonContainer().setRelativeLocation(
				new Vector2f((BUTTON_WIDTH / 2) + (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		disabledRecordStopButton.getButtonContainer().setVisible(false);

		enabledPlaybackStartButton = new RecorderButton(stage,
				"enabledPlaybackStartButton", "Start Playback", true);
		enabledPlaybackStartButton.getButtonContainer().setRelativeLocation(
				new Vector2f((-BUTTON_WIDTH / 2) - (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		...
		enabledPlaybackStartButton.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorPressed(MultiTouchCursorEvent event) {
		...
		disabledPlaybackStartButton = new RecorderButton(stage,
				"disabledPlaybackStartButton", "Start Playback", false);
		disabledPlaybackStartButton.getButtonContainer().setRelativeLocation(
				new Vector2f((-BUTTON_WIDTH / 2) - (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		disabledPlaybackStartButton.getButtonContainer().setVisible(false);

		enabledPlaybackStopButton = new RecorderButton(stage,
				"enabledRecordStopButton", "Stop Playback", true);
		enabledPlaybackStopButton.getButtonContainer().setRelativeLocation(
				new Vector2f((BUTTON_WIDTH / 2) + (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		...
		enabledPlaybackStopButton.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorPressed(MultiTouchCursorEvent event) {
		...
		disabledPlaybackStopButton = new RecorderButton(stage,
				"disabledRecordStopButton", "Stop Playback", false);
		disabledPlaybackStopButton.getButtonContainer().setRelativeLocation(
				new Vector2f((BUTTON_WIDTH / 2) + (BUTTON_GAP / 2),
						(HEIGHT / 2) - (BUTTON_HEIGHT / 2) - (BUTTON_GAP / 2)));
		...
		enabledOkButton = stage.getContentFactory().create(IButtonbox.class,
				"enabledOkButton", UUID.randomUUID());
		enabledOkButton.setText("Submit",
				MuseumAppPreferences.getRecorderBackgroundColour(),
				MuseumAppPreferences.getRecorderActiveButtonBorderColour(),
				MuseumAppPreferences.getRecorderActiveButtonFontColour(),
				BUTTON_WIDTH, BUTTON_HEIGHT / 2, stage);
		enabledOkButton.setRelativeLocation(new Vector2f((-enabledOkButton
				.getWidth() / 2) - (BUTTON_GAP / 2), (-HEIGHT / 2)
				+ (enabledOkButton.getHeight() / 2) + (BUTTON_GAP / 2)));
		enabledOkButton.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorPressed(MultiTouchCursorEvent event) {
						saveRecording();
		...
		disabledOkButton = stage.getContentFactory().create(IButtonbox.class,
				"disabledOkButton", UUID.randomUUID());
		disabledOkButton.setText("Submit",
				MuseumAppPreferences.getRecorderBackgroundColour(),
				MuseumAppPreferences.getRecorderInactiveButtonBorderColour(),
				MuseumAppPreferences.getRecorderInactiveButtonFontColour(),
				BUTTON_WIDTH, BUTTON_HEIGHT / 2, stage);
		disabledOkButton.setRelativeLocation(new Vector2f((-disabledOkButton
				.getWidth() / 2) - (BUTTON_GAP / 2), (-HEIGHT / 2)
				+ (disabledOkButton.getHeight() / 2) + (BUTTON_GAP / 2)));
		disabledOkButton.setVisible(false);

		IButtonbox cancelButton = stage.getContentFactory().create(
				IButtonbox.class, "cancelButton", UUID.randomUUID());
		...
		cancelButton.setRelativeLocation(new Vector2f(
				(cancelButton.getWidth() / 2) + (BUTTON_GAP / 2), (-HEIGHT / 2)
						+ (cancelButton.getHeight() / 2) + (BUTTON_GAP / 2)));
		...
		textLabel.setRelativeLocation(new Vector2f(0, (-HEIGHT / 2)
				+ enabledOkButton.getHeight() + BUTTON_GAP
				+ (remainingHeight / 2)));
		textLabel.setMovable(false);

		ICachableImage listener = stage.getContentFactory().create(
				ICachableImage.class, "listener", UUID.randomUUID());
	...
	private String generateName() {
		Date date = new Date();
		return DATE_FORMAT.format(date) + " " + name + " "
	...
	private void saveRecording() {
		File recordingsFolder = new File(folder.getAbsolutePath()
				+ File.separator + EntityManager.RECORDINGS);
		if (!recordingsFolder.isDirectory()) {
			recordingsFolder.mkdir();
		}
		File approvedRecordingsFolder = new File(
				recordingsFolder.getAbsolutePath() + File.separator
		...
		File userGeneratedFile = new File(recordingsFolder.getAbsolutePath()
				+ File.separator + id + ".wav");

		try {
			SettingsUtil.copyFile(audioController.getAudioFile(),
					userGeneratedFile);
		...
		if (MuseumAppPreferences.areMetricsEnabled()) {
			new MetricGUI(stage, new File(recordingsFolder.getAbsolutePath()),
					userGeneratedFile, id, prompt);
		} else {
			try {
				new TextEntryGUI(stage, new File(
						recordingsFolder.getAbsolutePath()), id, prompt);
