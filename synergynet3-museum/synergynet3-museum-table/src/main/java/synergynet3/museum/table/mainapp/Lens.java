...
public class Lens {
	...
	public Lens(IStage stageIn, String[] lensesIn, POIManager mapManagerIn) throws ContentTypeNotBoundException{
		stage = stageIn;
		...
		for (int i = 0; i < lenses.length; i++){
			...
			filters[i] = stage.getContentFactory().create(IColourRectangle.class, "filter", UUID.randomUUID());
			filters[i].enableTransparency();
			filters[i].setGradientBackground(new Gradient(colour, colour, GradientDirection.DIAGONAL));
			filters[i].setSize(WIDTH, HEIGHT + SETTINGS_HEIGHT);
			filters[i].setRelativeLocation(new Vector2f(0, -SETTINGS_HEIGHT/2));
			filters[i].setInteractionEnabled(false);
			filters[i].setVisible(false);
			container.addItem(filters[i]);
		}
		
		IImage bgImage = stage.getContentFactory().create(IImage.class, "border", UUID.randomUUID());
		bgImage.setImage(getBackgroundImage(MuseumAppPreferences.getLensBackgroundColour()));		
		...
		ageLabel = stage.getContentFactory().create(IMutableLabel.class, "textLabel", UUID.randomUUID());
		ageLabel.setFont(FontUtil.getFont(MuseumAppPreferences.getLensFontColour()));			
		ageLabel.setRelativeLocation(new Vector2f(0, -HEIGHT/2 - SETTINGS_HEIGHT/2 + 2f));
		...
		IImage listener = stage.getContentFactory().create(IImage.class, "listener", UUID.randomUUID());
		listener.setSize(WIDTH + (BORDER_WIDTH * 2), HEIGHT + SETTINGS_HEIGHT + (BORDER_WIDTH * 2));
		...
		final IgnoreDoubleClick clicker = new IgnoreDoubleClick(500){
			...
			public void onAction(MultiTouchCursorEvent event) {
				...
				for (IImage poi : mapManager.getPOIs()){		
					...
					boolean greaterThanMinX = eventLoc.x > poi.getWorldLocation().x - poi.getWidth()/2;
					boolean lessThanMaxX = eventLoc.x < poi.getWorldLocation().x + poi.getWidth()/2;					
					boolean greaterThanMinY = eventLoc.y > poi.getWorldLocation().y - poi.getHeight()/2;
					boolean lessThanMaxY = eventLoc.y < poi.getWorldLocation().y + poi.getHeight()/2;
					
					if (greaterThanMinX && lessThanMaxX && greaterThanMinY && lessThanMaxY){
				...
				for (LensVisiblePOI poi : mapManager.getlensVisiblePOIs()){
					...
					boolean greaterThanMinX = eventLoc.x > poi.getPoi().getWorldLocation().x - poi.getPoi().getWidth()/2;
					boolean lessThanMaxX = eventLoc.x < poi.getPoi().getWorldLocation().x + poi.getPoi().getWidth()/2;					
					boolean greaterThanMinY = eventLoc.y > poi.getPoi().getWorldLocation().y - poi.getPoi().getHeight()/2;
					boolean lessThanMaxY = eventLoc.y < poi.getPoi().getWorldLocation().y + poi.getPoi().getHeight()/2;
					
					if (greaterThanMinX && lessThanMaxX && greaterThanMinY && lessThanMaxY){
						poi.getPoi().getMultiTouchDispatcher().cursorPressed(event);						
					}
				}					
			}
		};
		...
		if (lenses.length > 1){
			...
			IImage leftButton = stage.getContentFactory().create(IImage.class, "down", UUID.randomUUID());
			leftButton.setImage(getArrowImage(MuseumAppPreferences.getLensArrowColour()));		
			leftButton.setSize(SETTINGS_HEIGHT/2, SETTINGS_HEIGHT/2);	
			leftButton.setRelativeLocation(new Vector2f(-WIDTH/2 + SETTINGS_HEIGHT/2, -HEIGHT/2 - SETTINGS_HEIGHT/2));	
			...
			IImage rightButton = stage.getContentFactory().create(IImage.class, "up", UUID.randomUUID());
			rightButton.setRelativeRotation(FastMath.DEG_TO_RAD * 180);
			rightButton.setImage(getArrowImage(MuseumAppPreferences.getLensArrowColour()));		
			rightButton.setSize(SETTINGS_HEIGHT/2, SETTINGS_HEIGHT/2);	
			rightButton.setRelativeLocation(new Vector2f(WIDTH/2 - SETTINGS_HEIGHT/2, -HEIGHT/2 - SETTINGS_HEIGHT/2));	
		...
		IImage closeButton = stage.getContentFactory().create(IImage.class, "closeButton", UUID.randomUUID());
		closeButton.setImage(ImageUtils.getImage(MuseumAppPreferences.getLensCloseColour(), ImageUtils.RESOURCE_DIR + "entitybuttons/close/", "_close_button.png"));
		closeButton.setSize(LabelGenerator.TEXT_HEIGHT/2, LabelGenerator.TEXT_HEIGHT/2);	
		closeButton.setRelativeLocation(new Vector2f(WIDTH/2 - closeButton.getWidth()/2, HEIGHT/2 - closeButton.getHeight()/2));
		closeButton.getMultiTouchDispatcher().addListener(new MultiTouchEventAdapter() {
			@Override
			public void cursorClicked(MultiTouchCursorEvent event) {	
				destroy();
	...
	private String getBackgroundImage(String colour){
		return ImageUtils.getImage(colour, ImageUtils.RESOURCE_DIR + "lensbackgrounds/", "_lens_bg.png");
	}	
	
	public boolean isWithinFilter(Vector2f vec){
		if (container.getRelativeLocation().distance(vec) < ((WIDTH/2)* container.getRelativeScale())){
			return true;
		}
		return false;
	}
