...
public class LabelGenerator {	
	...
	/** The Constant LINK_SCALE. */
	private static final float LINK_SCALE = 0.65f;

	/** The Constant NAME_SCALE. */
	 ...
	 */
	public static IItem generateFact(String fact, IStage stage)
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	public static IItem generateLink(final String link, final String parent,
			IStage stage, final MuseumApp app)
			throws ContentTypeNotBoundException {
		...
		final ITextbox textItem = stage.getContentFactory().create(
				ITextbox.class, "fact", UUID.randomUUID());
		textItem.setMovable(true);
		textItem.setColours(MuseumAppPreferences.getEntityBackgroundColour(),
				MuseumAppPreferences.getEntityBorderColour(),
				MuseumAppPreferences.getEntityFontColour());
		...
		textItem.getBackground().setSize(
				textItem.getBackground().getWidth() + TEXT_HEIGHT,
				textItem.getHeight());
		textItem.getBackground().setRelativeLocation(
				new Vector2f(TEXT_HEIGHT / 2, 0));
		...
		circle.setImage(ImageUtils.getImage(
				MuseumAppPreferences.getEntityLinkButtonBackgroundColour(),
				ImageUtils.RESOURCE_DIR + "entitybuttons/link/",
				"_link_button.png"));
		...
		circle.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					...
					public void cursorClicked(MultiTouchCursorEvent event) {
						app.focusShift(parent, link,
								textItem.getRelativeLocation());
					}
				});
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	public static ITextbox generateName(final String name, IStage stage,
			final MuseumApp app) throws ContentTypeNotBoundException {
		...
		textItem.getBackground().setSize(
				textItem.getBackground().getWidth() + TEXT_HEIGHT, TEXT_HEIGHT);
		textItem.getBackground().setRelativeLocation(
				new Vector2f(TEXT_HEIGHT / 2, 0));
		textItem.getTextBorder().setSize(textItem.getBackground().getWidth(),
				TEXT_HEIGHT);
		...
		IImage circle = stage.getContentFactory().create(IImage.class,
				"circleBackground", UUID.randomUUID());
		circle.setImage(ImageUtils.getImage(
				MuseumAppPreferences.getEntityCloseButtonBackgroundColour(),
