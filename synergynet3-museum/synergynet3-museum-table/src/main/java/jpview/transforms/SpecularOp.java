...
public final class SpecularOp implements PixelTransformOp
{
	...
	private void LRGBXform(int[] pixels, PTM ptm, int mouseX, int mouseY)
	{
		...
		final float[] L = Utils.normalize3(new float[]
		{ u, v, _w });
		final float[] V = new float[]
		{ 0, 0, 1 };
		final float[] H = Utils.normalize3(Utils.scalarMult3(Utils.vecSum3(L, V), 0.5f));
		...
		for (int i = 0; i < localPixels.length; i++)
		{
			...
			intensity = ((coeff[i][0] * _uu) >> 8) + ((coeff[i][1] * _vv) >> 8) + ((coeff[i][2] * _uv) >> 8) + ((coeff[i][3] * _u) >> 8) + ((coeff[i][4] * _v) >> 8) + (coeff[i][5]);
			...
			red = Math.round(((((red * intensity * KDIFF) >> 8) * _er) >> 8) + (_er * k * KSPEC * intensity));

			green = Math.round(((((green * intensity * KDIFF) >> 8) * _eg) >> 8) + (_eg * k * KSPEC * intensity));
			blue = Math.round(((((blue * intensity * KDIFF) >> 8) * _eb) >> 8) + (_eb * k * KSPEC * intensity));
	...
	private void RGBXform(int[] pixels, PTM ptm, int mouseX, int mouseY)
	{
		...
		for (int i = 0; i < localPixels.length; i++)
		{
			...
			red = ((coeff[0][i][0] * _uu) >> 8) + ((coeff[0][i][1] * _vv) >> 8) + ((coeff[0][i][2] * _uv) >> 8) + ((coeff[0][i][3] * _u) >> 8) + ((coeff[0][i][4] * _v) >> 8) + (coeff[0][i][5]);

			green = ((coeff[1][i][0] * _uu) >> 8) + ((coeff[1][i][1] * _vv) >> 8) + ((coeff[1][i][2] * _uv) >> 8) + ((coeff[1][i][3] * _u) >> 8) + ((coeff[1][i][4] * _v) >> 8) + (coeff[1][i][5]);

			blue = ((coeff[2][i][0] * _uu) >> 8) + ((coeff[2][i][1] * _vv) >> 8) + ((coeff[2][i][2] * _uv) >> 8) + ((coeff[2][i][3] * _u) >> 8) + ((coeff[2][i][4] * _v) >> 8) + (coeff[2][i][5]);

			red = Math.round((((red * KDIFF) >> 8) * N_red.dot(_L)) + (k_red * KSPEC * red));

			green = Math.round((((green * KDIFF) >> 8) * N_green.dot(_L)) + (k_green * KSPEC * green));

			blue = Math.round((((blue * KDIFF) >> 8) * N_blue.dot(_L)) + (k_blue * KSPEC * blue));
