...
public class Utils
{
	...
	{
		BufferedImage bi = new BufferedImage(scanline, pixels.length / scanline, BufferedImage.TYPE_INT_RGB);
	 ...
	 * @return the buffered image
	 */
	public static BufferedImage gaussianBlur(BufferedImage image, int kSize, float sigma)
	{
		BufferedImage target = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_RGB);
		Kernel kernel = new Kernel(kSize, kSize, new GaussianKernel(kSize, sigma).getKernel());
		ConvolveOp cop = new ConvolveOp(kernel);
		cop.filter(image, target);
		return target;
	}
	...
	public static void main(String[] args)
	{
		try
		{
			...
			{ .4, -.9, 0 };
			double[] incedent = new double[]
			{ 0, 0, 1 };
			normal = Utils.normalize3(normal);
			incedent = Utils.normalize3(incedent);
			double[] reflected = Utils.normalize3(Utils.reflect(normal, incedent));
