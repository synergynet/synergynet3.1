...
public class TargetSetterPanel extends VerticalPanel {
	...
	protected void updateList() {
		...
		for(int row = 0; row < orderedTables.size(); row++) {
			...
			final int index = row;
			up.addClickHandler(new ClickHandler() {  public void onClick(ClickEvent event) { moveUp(index); updateList();} });
			Button down = new Button("down");
			down.addClickHandler(new ClickHandler() {  public void onClick(ClickEvent event) { moveDown(index); updateList(); } });
			bpanel.add(up);
			bpanel.add(down);
