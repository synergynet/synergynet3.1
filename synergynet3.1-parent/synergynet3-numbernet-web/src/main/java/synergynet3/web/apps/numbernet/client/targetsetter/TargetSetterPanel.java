...
public class TargetSetterPanel extends VerticalPanel {
	...
	public TargetSetterPanel() {
		...
		chckbxHideIncorrect.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				shouldChangeDisplayOfIncorrectExpressions(!chckbxHideIncorrect
						.getValue());
			}
		});
		...
		chckbxHideCorrect.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				shouldChangeDisplayOfCorrectExpressions(!chckbxHideCorrect
						.getValue());
			}
		});

		final CheckBox chckbxHideOthersIncorrect = new CheckBox(
				"Hide others' incorrect");
		chckbxHideOthersIncorrect.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				shouldChangeDisplayOfOthersIncorrectExpressions(!chckbxHideOthersIncorrect
						.getValue());
			}
		});
		verticalPanel.add(chckbxHideOthersIncorrect);

		final CheckBox chckbxHideOthersCorrect = new CheckBox(
				"Hide others' correct");
		chckbxHideOthersCorrect.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				shouldChangeDisplayOfOthersCorrectExpressions(!chckbxHideOthersCorrect
						.getValue());
			}
		});
	...
	protected void changeCalculatorVisibility(Boolean value) {
		NumberNetService.Util.getInstance().setCalculatorVisibility(value,
	...
	protected void moveDown(int index) {
		System.out.println("down on " + index + " with list of "
				+ orderedTables.size() + " items");
	...
	protected void moveUp(int index) {
		System.out.println("up on " + index + " with list of "
				+ orderedTables.size() + " items.");
		TableTarget taken = orderedTables.remove(index);
		if ((index - 1) < 0) {
	...
	protected void rotateAndSetTargets() {
		if (graphingModeCheckBox.getValue()) {
			new MessageDialogBox(
					"Cannot change targets while in graphing mode.").show();
			return;
		}
		...
		for (int row = 0; row < flexTable.getRowCount(); row++) {
			...
			String tableFrom = ((Label) flexTable.getWidget(targetIndex, 1))
					.getText();
			double target = Double.parseDouble(targets.get(tableFrom));
			targetsList.add(new TableTarget(table, target));
		}

		NumberNetService.Util.getInstance().rotateTableContentAndTargets(
				targetsList, new AsyncCallback<Void>() {
	 ...
	 *
	 * @param value the new all tables should respond to touches
	 */
	protected void setAllTablesShouldRespondToTouches(boolean value) {
		NumberNetService.Util.getInstance().setTableInputEnabled(value,
				new AsyncCallback<Void>() {
	 ...
	 *
	 * @param value the new graphing mode enabled
	 */
	protected void setGraphingModeEnabled(Boolean value) {
		NumberNetService.Util.getInstance().setGraphingModeEnabled(value,
				new AsyncCallback<Void>() {
					@Override
					public void onFailure(Throwable caught) {
						new MessageDialogBox(caught.getMessage()).show();
	...
	protected void setTargets() {
		...
		try {
			...
			for (int row = 0; row < flexTable.getRowCount(); row++) {
				String table = ((Label) flexTable.getWidget(row, 1)).getText();
				double target = Double.parseDouble(((TextBox) flexTable
						.getWidget(row, 2)).getText());
				targetsList.add(new TableTarget(table, target));
			}

			NumberNetService.Util.getInstance().setTableTargets(targetsList,
					new AsyncCallback<Void>() {
						@Override
						public void onFailure(Throwable caught) {
							...
							new MessageDialogBox("Error." + caught.getMessage())
									.show();
						}
			...
			new MessageDialogBox(
					"Please make sure all tables have a numerical target.")
					.show();
		}
	}
	 ...
	 */
	protected void shouldChangeDisplayOfCorrectExpressions(
			boolean shouldBeVisible) {
		NumberNetService.Util.getInstance()
				.setCorrectExpressionsVisibleForAllTables(shouldBeVisible,
						new AsyncCallback<Void>() {
							@Override
							public void onFailure(Throwable caught) {
	 ...
	 * @param shouldBeVisible the should be visible
	 */
	protected void shouldChangeDisplayOfIncorrectExpressions(
			boolean shouldBeVisible) {
		NumberNetService.Util.getInstance()
				.setIncorrectExpressionsVisibleForAllTables(shouldBeVisible,
						new AsyncCallback<Void>() {
							@Override
							public void onFailure(Throwable caught) {
								new MessageDialogBox(caught.getMessage())
	 ...
	 * @param shouldBeVisible the should be visible
	 */
	protected void shouldChangeDisplayOfOthersCorrectExpressions(
			boolean shouldBeVisible) {
		NumberNetService.Util.getInstance()
				.setOthersCorrectExpressionsVisibleForAllTables(
						shouldBeVisible, new AsyncCallback<Void>() {
							@Override
							public void onFailure(Throwable caught) {
								new MessageDialogBox(caught.getMessage())
										.show();
	 ...
	 * @param shouldBeVisible the should be visible
	 */
	protected void shouldChangeDisplayOfOthersIncorrectExpressions(
			boolean shouldBeVisible) {
		NumberNetService.Util.getInstance()
	...
	protected void shouldChangeDisplayOfScores(boolean shouldBeVisible) {
		NumberNetService.Util.getInstance().setScoresVisibleForAllTables(
				shouldBeVisible, new AsyncCallback<Void>() {
					@Override
					public void onFailure(Throwable caught) {
						new MessageDialogBox(caught.getMessage()).show();
	 ...
	 */
	protected void updateOrderedTablesListWithNewTableList(
			List<TableTarget> result) {
