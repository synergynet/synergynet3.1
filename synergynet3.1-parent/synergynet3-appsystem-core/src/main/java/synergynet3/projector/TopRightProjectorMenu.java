...
public class TopRightProjectorMenu {
	...
	private static final Logger log = Logger
			.getLogger(TopRightProjectorMenu.class.getName());
	...
	private static final String[] tableActions = { "Send content to",
			"Show content from", "Clear", "Freeze/Unfreeze",
			"View Snapshots of" };

	/** The Constant tableSelections. */
	private static final String[] tableSelections = { "all tables",
			"these tables:" };
	...
	private void addOrRemoveTableButtons() {
		...
		selectionLabel.getTextLabel().setText(
				tableSelections[tableSelectionIndex]);

		float x = X_TABLE_BUTTONS_OFFSET;
		float y = Y_TABLE_BUTTONS_OFFSET;

		if (tableSelectionIndex == 1) {
			for (final String table : AppSystemControlComms.get()
					.getTablesList()) {
				try {
					final IToggleButtonbox tableButton = stage
							.getContentFactory().create(IToggleButtonbox.class,
									"tableButtonOff", UUID.randomUUID());
					tableButton.setText(table.toLowerCase(), ColorRGBA.Black,
							ColorRGBA.Gray, FontColour.White,
							table.toLowerCase(), ColorRGBA.Gray,
							ColorRGBA.White, FontColour.White, BUTTON_SIZE * 3,
							BUTTON_SIZE, stage);
					tableButton.setRelativeLocation(new Vector2f(x, y));
					tableButtons.add(tableButton);
					topRightControls.addItem(tableButton);

					tableButton.getListener().getMultiTouchDispatcher()
	...
	private void generateActionsButtons() throws ContentTypeNotBoundException {
		...
		actionsLabel = stage.getContentFactory().create(IButtonbox.class,
				"textLabel", UUID.randomUUID());
		actionsLabel.setText(tableActions[tableActionIndex], ColorRGBA.Black,
				ColorRGBA.Black, FontColour.White, 285, 40, stage);
		actionsLabel.setRelativeLocation(new Vector2f(0, y));
		actionsLabel.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						...
						actionsLabel.getTextLabel().setText(
								tableActions[tableActionIndex]);
					}
				});
		topRightControls.addItem(actionsLabel);

		ICachableImage actionsScrollUp = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		actionsScrollUp.getMultiTouchDispatcher().addListener(
		...
		ICachableImage actionsScrollDown = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		actionsScrollDown.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableActionIndex--;
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private void generateConfirmationButtons()
		...
		okButton.setText("OK", ColorRGBA.Black, ColorRGBA.White,
				FontColour.White, BUTTON_SIZE * 3, BUTTON_SIZE, stage);
		...
		IButtonbox cancelButton = stage.getContentFactory().create(
				IButtonbox.class, "button", UUID.randomUUID());
		cancelButton.setText("Cancel", ColorRGBA.Black, ColorRGBA.White,
				FontColour.White, BUTTON_SIZE * 3, BUTTON_SIZE, stage);
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private void generateTablesSelectionButtons()
			throws ContentTypeNotBoundException {
		float y = 75;

		selectionLabel = stage.getContentFactory().create(IButtonbox.class,
				"textLabel", UUID.randomUUID());
		selectionLabel.setText(tableSelections[tableSelectionIndex],
				ColorRGBA.Black, ColorRGBA.Black, FontColour.White, 285, 40,
				stage);
		selectionLabel.setRelativeLocation(new Vector2f(0, y));
		selectionLabel.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableSelectionIndex++;
		...
		ICachableImage selectionScrollUp = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		selectionScrollUp.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableSelectionIndex++;
		...
		ICachableImage selectionScrollDown = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		selectionScrollDown.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableSelectionIndex--;
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private void generateTablesViewedButtons()
			throws ContentTypeNotBoundException {
		float y = Y_TABLE_BUTTONS_OFFSET - ((Y_TABLE_GAP * (Y_LIMIT - 1)) / 2);

		tablesViewedScrollUp = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		tablesViewedScrollUp.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableButtonFrame++;
						if (tableButtonFrame > (int) (FastMath
								.ceil((tableButtons.size() - 1)
										/ (X_LIMIT * Y_LIMIT)))) {
							tableButtonFrame = 0;
						}
						manageTableButtonVisiblity();
		...
		tablesViewedScrollDown = stage.getContentFactory().create(
				ICachableImage.class, "scrollUp", UUID.randomUUID());
		...
		tablesViewedScrollDown.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
						tableButtonFrame--;
						if (tableButtonFrame < 0) {
							tableButtonFrame = (int) (FastMath
									.ceil((tableButtons.size() - 1)
											/ (X_LIMIT * Y_LIMIT)));
						}
						manageTableButtonVisiblity();
	...
	private void generateTopRightButton() throws ContentTypeNotBoundException {
		topRightCornerButton = stage.getContentFactory().create(IImage.class,
				"cornerButton", UUID.randomUUID());
		topRightCornerButton.setImage(CORNER_ICON_LOC_OFF);
		topRightCornerButton.setSize(BUTTON_SIZE * 2, BUTTON_SIZE * 2);
		topRightCornerButton.setRelativeLocation(new Vector2f(stage
				.getDisplayWidth() / 2, stage.getDisplayHeight() / 2));

		topRightCornerButton.getMultiTouchDispatcher().addListener(
				new MultiTouchEventAdapter() {
					@Override
	...
	private void generateTopRightMenu() throws ContentTypeNotBoundException {
		...
		topRightControls = stage.getContentFactory().create(IContainer.class,
	...
	private void initialiseMenu() {
		...
		topRightControls.setRelativeLocation(new Vector2f((stage
				.getDisplayWidth() / 2) - (WIDTH / 2) - (BUTTON_SIZE - OFFSET),
				(stage.getDisplayHeight() / 2) - (HEIGHT / 2)
						- (BUTTON_SIZE - OFFSET)));
		topRightControls.setVisible(false);
	}
		...
		switch (tableActionIndex) {
			...
			case 1:
				for (String table : tablesToSendTo) {
					if (table.equals(DevicesSelected.ALL_TABLES_ID)) {
						AppSystemControlComms.get()
								.allTablesSendContentsToProjectors(
										projectorsToSendTo);
					} else {
						AppSystemControlComms.get()
			...
			case 2:
				for (String table : tablesToSendTo) {
					if (table.equals(DevicesSelected.ALL_TABLES_ID)) {
						AppSystemControlComms.get()
								.allTablesRemoveAdditionalContent(
