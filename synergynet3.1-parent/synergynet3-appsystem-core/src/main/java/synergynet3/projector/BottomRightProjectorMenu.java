...
public class BottomRightProjectorMenu {
	...
	private static final Logger log = Logger
			.getLogger(BottomRightProjectorMenu.class.getName());
	...
	/**
	 * Instantiates a new bottom right projector menu.
	 *
	...
	/**
	 * Generate bottom right button.
	 *
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private void generateBottomRightButton()
			throws ContentTypeNotBoundException {
		bottomRightCornerButton = stage.getContentFactory().create(
				IImage.class, "cornerButton", UUID.randomUUID());
		bottomRightCornerButton.setImage(CORNER_ICON_LOC_OFF);
		bottomRightCornerButton.setSize(BUTTON_SIZE * 2, BUTTON_SIZE * 2);
		bottomRightCornerButton.setRelativeLocation(new Vector2f(stage
				.getDisplayWidth() / 2, -stage.getDisplayHeight() / 2));

		bottomRightCornerButton.getMultiTouchDispatcher().addListener(
					...
					public void cursorPressed(MultiTouchCursorEvent event) {
						if (bottomRightControls.isVisible()) {
							...
							stage.getZOrderManager().bringToTop(
									bottomRightControls);
							stage.getZOrderManager().sendToBottom(
									bottomRightCornerButton);
							bottomRightCornerButton
									.setImage(CORNER_ICON_LOC_ON);
						}
					}
				});
	...
	/**
	 * Generate bottom right buttons.
	 *
	 * @return the array list
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private ArrayList<IItem> generateBottomRightButtons()
			throws ContentTypeNotBoundException {
		ArrayList<IItem> buttons = new ArrayList<IItem>();

		IButtonbox alignButton = stage.getContentFactory().create(
				IButtonbox.class, "button", UUID.randomUUID());
		alignButton.setText("Align", ColorRGBA.Black, ColorRGBA.White,
				FontColour.White, BUTTON_SIZE * 3, BUTTON_SIZE, stage);
		alignButton.getListener().getMultiTouchDispatcher()
		...
		IButtonbox clearButton = stage.getContentFactory().create(
				IButtonbox.class, "button", UUID.randomUUID());
		clearButton.setText("Clear", ColorRGBA.Black, ColorRGBA.White,
				FontColour.White, BUTTON_SIZE * 3, BUTTON_SIZE, stage);
		clearButton.getListener().getMultiTouchDispatcher()
				.addListener(new MultiTouchEventAdapter() {
					@Override
					public void cursorClicked(MultiTouchCursorEvent event) {
	...
	private void generateBottomRightMenu() throws ContentTypeNotBoundException {
		...
		bottomRightControls = stage.getContentFactory().create(
				IContainer.class, "wrapper", UUID.randomUUID());
		...
		bottomRightControls.setRelativeLocation(new Vector2f((stage
