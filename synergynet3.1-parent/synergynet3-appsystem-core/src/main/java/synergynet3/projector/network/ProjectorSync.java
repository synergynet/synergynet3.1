...
public class ProjectorSync {
	...
	private DistributedPropertyChangedAction<ArrayList<ContentTransferedMessage>> receiveContentAction = new DistributedPropertyChangedAction<ArrayList<ContentTransferedMessage>>() {
		...
		public void distributedPropertyDidChange(Member member,
			...
			if (newValue.size() > 0) {
				snProjectorNode.onContentArrival(newValue);
			}
		}
	};
	 ...
	 */
	public ProjectorSync(ProjectorDeviceControl synergynetProjectorControl,
			SynergyNetProjector snProjectorNode) {
	 ...
	 */
	public void stop() {
		synergynetProjectorControl.getProjectorClearControl()
				.unregisterChangeListener(clearAction);
		synergynetProjectorControl.getProjectorAlignControl()
				.unregisterChangeListener(alignAction);
		synergynetProjectorControl.getSendContentsToTablesControl()
				.unregisterChangeListener(sendProjectedContentsAction);
		synergynetProjectorControl.getContentTransferToProjectorControl()
				.unregisterChangeListener(receiveContentAction);
	}
	...
	private void addSync() {
		synergynetProjectorControl.getProjectorClearControl()
				.registerChangeListener(clearAction);
		synergynetProjectorControl.getProjectorAlignControl()
				.registerChangeListener(alignAction);
		synergynetProjectorControl.getSendContentsToTablesControl()
				.registerChangeListener(sendProjectedContentsAction);
		synergynetProjectorControl.getContentTransferToProjectorControl()
				.registerChangeListener(receiveContentAction);
	}
