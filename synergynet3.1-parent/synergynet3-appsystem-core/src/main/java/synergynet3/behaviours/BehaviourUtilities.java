...
public class BehaviourUtilities {
	...
	/**
	 * Defines behaviour of network flick transfers - by default it is realistic (i.e. travels on and off the screen edge).
	 * Instant mode is an alternative behaviour where transferred items will instantly be transferred on bounce.
	 */
	...
	*/
	public static IItem onFlickArrival(final FlickMessage message, IStage stage, String tableIdentity, float deceleration){			
		...
		if (message.getTargetTableID().equals(tableIdentity)){
			...
			item = ItemCaching.reconstructItem(message.getGalleryItem(), stage, 
					CacheOrganisation.TRANSFER_DIR + File.separator + CacheOrganisation.NETWORK_FLICK_DIR, deceleration);
			...
			if (NetworkFlickLogging.LOGGING_ENABLED)NetworkFlickLogging.generateFlickArrivalMessage(item.getName(), message.getSourceTableID(), loc);				
