...
public class BehaviourUtilities {
	 ...
	 *            arrival.
	 * @param stage The environement's main stage.
	 * @param tableIdentity Table's ID on the network cluster.
	 * @param deceleration Rate of flick motion reduction.
	 */
	public static IItem onFlickArrival(final FlickMessage message,
			IStage stage, String tableIdentity, float deceleration) {
		...
		if (message.getTargetTableID().equals(tableIdentity)) {
			...
			item = ItemCaching
					.reconstructItem(message.getGalleryItem(), stage,
							CacheOrganisation.TRANSFER_DIR + File.separator
									+ CacheOrganisation.NETWORK_FLICK_DIR,
							deceleration);

			item.setRelativeLocation(new Vector2f(SynergyNetPositioning
					.getPixelValue(message.getXinMetres()),
					SynergyNetPositioning.getPixelValue(message.getYinMetres())));
			...
			if (FeedbackSystem.isItemFeedbackContainer(item)) {
				FeedbackSystem.getFeedbackContainer(item).getContainedItem()
						.setVisible(true);
			...
			if (NetworkFlickLogging.LOGGING_ENABLED) {
				NetworkFlickLogging.generateFlickArrivalMessage(item.getName(),
						message.getSourceTableID(), loc);
			}
			...
			new PerformActionOnAllDescendents(item, false, false) {
				...
				protected void actionOnDescendent(IItem child) {
					for (IBehaviour nf : behaviourMaker.getBehavior(child,
						...
						((NetworkFlickBehaviour) nf).flick(new Vector2f(message
								.getXDir(), message.getYDir()));
					}
				}
			};
		}
