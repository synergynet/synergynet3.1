...
/**
 * A collection of methods for the management of representations of other device
 * interfaces running instances of SynergyNetApp applications.
 */
public class VirtualTableUtilities {
	 ...
	 * cluster's interface. The method adds this in the same position as its
	 * real life counterpart in relation to the local device's interface
	 * position. The method then sends a structured message detailing the local
	 * device's interface position to the device represented by the spatial.
	 *
	 * @param tableID ID of the device of the interface represented on the
	 *            network cluster.
	 * @param location Location of the interface represented in the physical
	 *            environment relative to the local device's interface position.
	 * @param orientation Orientation of the interface represented in the
	 *            physical environment relative to the local device's interface
	 *            position.
	 * @param dimensions The width and height of the device represented's
	 *            interface.
	 * @param pixelWidth The width of the device represented's interface in
	 *            pixels.
	 */
	public static void addTable(String tableID, Vector2f location,
			float orientation, Vector2f dimensions, float pixelWidth,
			float metreWidth, IStage stage) {

		if (tableID == null) {
			return;
		}
		if (tableID.equals(SynergyNetApp.getTableIdentity())) {
			return;
		}
		...
		Box box = new Box(new Vector3f(), dimensions.x / 2, dimensions.y / 2,
				10);
		...
		Vector2f thisTableLocation = new Vector2f(
				SynergyNetPositioning.getPixelValue(SynergyNetApp.localDevicePosition
						.getXinMetres()),
				SynergyNetPositioning
						.getPixelValue(SynergyNetApp.localDevicePosition
								.getYinMetres()));
		...
		float localPixelsPerM = SynergyNetApp.localDevicePosition
				.getPixelWidth()
				/ SynergyNetApp.localDevicePosition.getWidthinMetres();
		...
		try {
			...
			IColourRectangle tableRep = stage.getContentFactory().create(
					IColourRectangle.class, "virtualTable", UUID.randomUUID());
			...
			if (colour == null) {
				...
				tableRep.setSolidBackgroundColour(ColourUtils
						.getColorRGBAFromColor(colour));
			}
			tableRep.setRelativeRotation(-orientation);
			tableRep.setRelativeLocation(new Vector2f(relativePosition.x,
					relativePosition.y));
			container.addItem(tableRep);
			container.setRelativeRotation(SynergyNetApp.localDevicePosition
					.getOrientation());
			stage.addItem(container);

			virtualTable.setLocalTranslation(tableRep.getWorldLocation().x,
					tableRep.getWorldLocation().y, 0);
			virtualTable.setLocalRotation(tableRep.getManipulableSpatial()
					.getWorldRotation());
	...
	public static void announceTablePositionToAll() {
		...
		AppSystemControlComms.get().allTablePositionUpdate(
	...
	public static void announceTablePositionToSpecificTable(String targetTableID) {
		AppSystemControlComms.get().specificTablePositionUpdate(
				SynergyNetApp.localDevicePosition, targetTableID);
	}
	 ...
	 * @return A String representing the ID of the device represented on the
	 *         network cluster.
	 */
	 ...
	 * the environment. The method also removes the entries from the collections
	 * of spatials, IDs and orientations of the other devices on the network
	 * cluster.
	 *
	...
	public static void removeTable(String tableID) {
		...
		if (otherTablesOrientation.containsKey(tableID)) {
			otherTablesOrientation.remove(tableID);
		}
		if (otherTablesScaleChanges.containsKey(tableID)) {
			otherTablesScaleChanges.remove(tableID);
		}
	...
	public static void removeTableFromAll() {
		SynergyNetPosition localTable = new SynergyNetPosition(
				SynergyNetApp.getTableIdentity(), true);
