...
/**
 * Behaviour for items which are intended to be capable of being flicked across the network.
 * Note that with this behaviour an item does not need inertiaBehaviour, inertia is supplied by this class.
 *
 */
public class NetworkFlickBehaviour implements IBehaviour, IMultiTouchEventListener {
	
	/**
	 * Collection of positions at which the item has currently been positioned when moved by a user.
	 */
	...
	* Repositions an item when flick using the JME animation system.
	...
	public void cursorChanged(MultiTouchCursorEvent event) {			
		if(!active ) return;		
		if (displayDimensions == null)displayDimensions = new Vector2f(stage.getDisplayWidth(), stage.getDisplayHeight());		
	 ...
	 * 
	 * @param vX X component of the velocity the item that the managed item is bouncing off.
	 * @param vY Y component of the velocity the item that the managed item is bouncing off.
	 * @param rp 
	 */
