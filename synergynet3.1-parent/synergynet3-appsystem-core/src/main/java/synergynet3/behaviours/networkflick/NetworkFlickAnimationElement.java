...
public class NetworkFlickAnimationElement extends AnimationElement {
	...
	/**
	 * Where the item is destined to arrive on a flick relative to the local table.
	 */
	 ...
	 * 
	 * @param drag The deceleration at which the managed item's momentum is reduced.
	 */
	 ...
	 * 
	 * @return A float representing the deceleration at which the managed item's momentum is reduced.
	 */
	...
	public void bounce(float vX, float vY, float tpf, RelativePosition bouncePosition){
		...
		if (boostNeeded)AnimationSystem.getInstance().add(this);
	...
	public void updateAnimationState(float tpf) {
		if (message == null){
			if (!finished){
				...
				if (item.getWorldLocation().x > stage.getDisplayWidth()){
					...
					if (currentVelocity.x > 0){
						if(!initiateTransfer())currentVelocity.setX(-currentVelocity.getX());
					}
				...
				if (!finished){
					if (item.getWorldLocation().y > stage.getDisplayHeight()){
						...
						if (currentVelocity.y > 0){
							if(!initiateTransfer())currentVelocity.setY(-currentVelocity.getY());									
						}
			...
			item.setWorldLocation(item.getWorldLocation().add(currentVelocity.mult(tpf)));
			if (exitLoc.distance(item.getWorldLocation()) > getMaxDimension() * (TRANSFER_DISTANCE_RATIO/2))transfer();			
		}
	}
	
	/**
	 * If the network flick feature is enabled by the current application this method
	 * creates a ray along the managed item's current trajectory to detect if another devices' interface lies in its path.
	 * If so the method then checks that the item has enough momentum to travel off screen.
	 * If this is so the method then constructs a structured method to be sent across the network to instigate a network flick.
	 * 
	 * @return A boolean value representing whether the managed item is to be transfered.
	 */
	private boolean initiateTransfer(){
		if (!SynergyNetApp.networkFlickEnabled) return false;
		if (((item instanceof IItemCachable) || FeedbackSystem.isItemFeedbackContainer(item) )&& FeedbackSystem.isItemFeedbackEligible(item)){			
			...
			for (Spatial table : VirtualTableUtilities.otherTables.values()){
				...
				if (results.size() > 0){
					...
					if (distance == -1 || distance > targetLocation.distance(item.getRelativeLocation())){
				    	...
				    	locationFromTargetTableCentre = new Vector2f(interactionLocation.x - table.getWorldTranslation().x, 
				    			interactionLocation.y - table.getWorldTranslation().y);
			...
			if (targetTable != null){				
				...
				Thread cachingThread = new Thread(new Runnable() {	  
					public void run() {	   
						...
						if (!id.equals(finalTargetTableID)){
							...
							message = new FlickMessage(finalTargetTableID, id, galleryItem, locationFromTargetTableCentreXinMetres, locationFromTargetTableCentreYinMetres,
									finalTargetAngle, finalTargetScale, newFlickDirection.x, newFlickDirection.y);
							
							if (BehaviourUtilities.FLICK_TYPE == FLICKTYPE.INSTANT)transfer();
						}	
				...
				if (!id.equals(targetTableID))return true;
			}
		}
	...
	/**
	 * Log the reflecting of an item off the boundary if it is not being transferred.
	 */
	private boolean bounceBack(){
		Vector2f loc = item.getWorldLocation();
		if (NetworkFlickLogging.LOGGING_ENABLED)NetworkFlickLogging.generateInaccurateBounceMessage(item.getName(), loc);
	...
	/**
	 * If a network flick message has been created this method will instigate its transmission
	 * through the cluster to the appropriate device.  The managed item is then removed from the application.
	 */
	private void transfer(){			
		
		Vector2f worldLoc = item.getWorldLocation();
		Vector2f localLoc = item.getRelativeLocation();
		if (NetworkFlickLogging.LOGGING_ENABLED)NetworkFlickLogging.generateFlickSentMessage(item.getName(), message.getTargetTableID(), worldLoc);
		finished = true;					
	...
	/**
	 * Checks to see if the managed item current has enough momentum to travel three times its max dimension.
	 * This is enough distance for the managed item to travel off screen after its collision with a boundary when transferred.
	 * 
	 * @return A boolean value representing whether the managed item has enough momentum to travel entirely off screen.
	 */
	private boolean withinStoppingDistance() {
		...
		Vector2f loc = item.getWorldLocation();
		if (NetworkFlickLogging.LOGGING_ENABLED)NetworkFlickLogging.generateLackOfMomentumBounceMessage(item.getName(), loc);
		if (NetworkFlickLogging.BOUNCE_LIMIT > 0){
			momentumBounces++;
	...
	/**
	 * Uses the managed item's bounding box to find the largest of its width or height.  If the managed item does not have a bounding box
	 * then a predetermined value is used.
	 * 
	 * @return A float representing the largest of the managed item's width or height.
	 */
	...
	/**
	 * Calculates the speed at which the managed item is currently travelling in pixels per second.
	 * 
	 * @return A float representing the speed at which the managed item is currently travelling in pixels per second.
	 */
	 ...
	 * 
	 * @param velocity The direction and momentum with which the managed item is to be flicked.
	 */
	...
	/**
	 * Sets the predetermined max dimension of the managed item.  This is used for items
	 * which may not have a bounding box.
	 * 
