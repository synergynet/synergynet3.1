...
public class NetworkFlickLogging {
	 ...
	 * @param loc the loc
	 */
	public static void generateFlickArrivalMessage(String itemName,
			String from, Vector2f loc) {
		FLICK_LOG += generateDateAndLocationText(loc) + itemName
				+ " arrived from " + from + "." + LOG_PARSE_TOKEN;
		ARRIVAL_COUNT++;
	}
	 ...
	 * @param loc the loc
	 */
	public static void generateFlickSentMessage(String itemName, String to,
			Vector2f loc) {
		FLICK_LOG += generateDateAndLocationText(loc) + itemName + " sent to "
				+ to + "." + LOG_PARSE_TOKEN;
		DEPARTURE_COUNT++;
	}
	 ...
	 * @param loc the loc
	 */
	public static void generateInaccurateBounceMessage(String itemName,
			Vector2f loc) {
		BOUNCE_LOG += generateDateAndLocationText(loc) + itemName
				+ " bounced due to an inaccurate flick." + LOG_PARSE_TOKEN;
	 ...
	 * @param loc the loc
	 */
	public static void generateLackOfMomentumBounceMessage(String itemName,
			Vector2f loc) {
		BOUNCE_LOG += generateDateAndLocationText(loc) + itemName
				+ " bounced due to a flick with a lack of momentum."
				+ LOG_PARSE_TOKEN;
	 ...
	 * @param loc the loc
	 */
	public static void generateRemovalDueToInaccurateBounceMessage(
			String itemName, Vector2f loc) {
		BOUNCE_LOG += generateDateAndLocationText(loc) + itemName
				+ " removed due to too many inaccurate flicks."
				+ LOG_PARSE_TOKEN;
		ARRIVAL_COUNT++;
	}
	 ...
	 * @param loc the loc
	 */
	public static void generateRemovalDueToLackOfMomentumBounceMessage(
			String itemName, Vector2f loc) {
		BOUNCE_LOG += generateDateAndLocationText(loc) + itemName
				+ " removed due to too many flicks with a lack of momentum."
				+ LOG_PARSE_TOKEN;
		ARRIVAL_COUNT++;
	}
	...
	private static String generateDateAndLocationText(Vector2f loc) {
		Date date = new Date();
		String timing = DATE_FORMAT.format(date) + " "
