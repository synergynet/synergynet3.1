...
abstract public class SynergyNetApp implements IMultiplicityApp, IScreenShotter, IMediaSearcher
{
	...
	/**
	 * Array of file paths used to record when a file has been made into an
	 * item.
	 */
	...
	/**
	 * Used to scale the size of imported media. Can be overwritten by
	 * applications extending SynergyNetApp.
	 */
	protected int importImageSizeLimit = 250;

	/**
	 * Flag to stop screenshots being generated while network variable are being
	 * initialised.
	 */
	protected boolean initialised = false;

	/**
	 * Roster of students to be logged in when an application extending
	 * SynergyNetApp is started.
	 */
	...
	/**
	 * Indicates whether an item representing the last screenshot made has been
	 * created yet.
	 */
	protected boolean screenShotAwaiting = false;

	/**
	 * Manager for thread checking the current device's corresponding directory
	 * in the shared cache for new files.
	 */
	 ...
	 * SynergyNetApp. This method first tidies any unused files from the
	 * synergynet cache. The method then sets up the default feedback types to
	 * be used in SynergyNetApps and records the display dimensions. The screen
	 * capture function is then initialised and login menus are created for any
	 * waiting students. Following this the controller which listens for
	 * appropriate changes in the HazelCast Cluster is initialised. The
	 ...
	 * traditional means. All students present art logged out, the media
	 * searches are stopped and the table announces its removal to the network.
