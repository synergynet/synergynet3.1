...
public class AdditionalSynergyNetUtilities
{
	 ...
	 * @param deceleration
	 *            The rate of the deceleration when flicked. A value of -1 will
	 *            make the item unflickable.
	...
	{
		if ((loc.x > (boxLoc.x - (dim.x / 2))) && (loc.x < (boxLoc.x + (dim.x / 2))) && (loc.y > (boxLoc.y - (dim.y / 2))) && (loc.y < (boxLoc.y + (dim.y / 2))))
		{
			return true;
		}
		return false;
	 ...
	 *            How far along an axis from the centre of the pile an item may
	 *            be positioned.
	 **/
