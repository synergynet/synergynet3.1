...
public class AdditionalSynergyNetUtilities {
	...
	/** Logger used to provide details on the current execution of the utilities offered by this class.*/
	protected static final Logger log = Logger.getLogger(AdditionalSynergyNetUtilities.class.getName());
	...
	private static final String[] TEXTURE_EXTENSIONS = {".jpg", ".JPG", ".jpeg", ".JPEG", ".gif", ".GIF", ".png", ".PNG"};
	...
	* @param x
	*          X location at which the pile of items should appear.
	* @param y
	*          Y location at which the pile of items should appear.
	* @param angleOffSet
	*          The degree to which items in the pile may be rotated.
	* @param positionOffset
	*          How far along an axis from the centre of the pile an item may be positioned.       
	**/
	public static void pile(IItem[] items, float x, float y, float angleOffSet, float positionOffset){
	...
	/**
	* Creates item for a file.  If the file has a wav or mp3 extension an audio playing item is created.
	* If the file has a jpg or png extension then an image item is created. 
	* Otherwise no item is created.  Any items created are made legible for feedback and network flick capable.
	* 
	* @param file
	*          The file to potentially create an item from.
	* @param stage
	*          Environment stage node
	* @param deceleration
	*          The rate of the deceleration when flicked.  A value of -1 will make the item unflickable.
	* @param importImageSizeLimit
	*          The max width or height of an item.     
	* @param borderColour
	*          The colour of the border around the item.  Can be invisible.
	*          
	* @return The Item generated for the given file, or null if the file is not valid.          
	*          
	**/
	public static IItem generateItemFromFile(File file, IStage stage, float deceleration, float importImageSizeLimit, ColorRGBA borderColour) {
		return generateItemFromFile(file, stage, deceleration, importImageSizeLimit, borderColour, -1, -1);
	}
	...
	*          The colour of the border around the item.  Can be invisible.
	* @param minScale
	*          The minimum scale the item can be scaled it.     
	* @param maxScale
	*          The maximum scale the item can be scaled it.           
	*          
	* @return The Item generated for the given file, or null if the file is not valid.          
	*          
	**/	
	public static IItem generateItemFromFile(File file, IStage stage, float deceleration, float importImageSizeLimit, ColorRGBA borderColour, float minScale, float maxScale) {
		IItem toReturn = null;
		
		BehaviourMaker behaviourMaker = stage.getBehaviourMaker();
		IContentFactory contentFactory = stage.getContentFactory();
				
		if (AUDIO_FILE_TYPE.isFileOfSearchType(file) || VIDEO_FILE_TYPE.isFileOfSearchType(file)){	
			try{
				IMediaPlayer mediaPlayer = stage.getContentFactory().create(IMediaPlayer.class, file.getName(), UUID.randomUUID());
				...
				if (width > importImageSizeLimit || height > importImageSizeLimit){
					if (width < height){
						float newWidth = width * (importImageSizeLimit / height);	
						mediaPlayer.setSize(newWidth, importImageSizeLimit);
					}else{
						float newHeight = height * (importImageSizeLimit / width);	
				...
				FeedbackSystem.registerAsFeedbackEligible(mediaPlayer, width, height, stage);
			} catch (Exception e) {
			...
			for (String ext: TEXTURE_EXTENSIONS){
				...
				File textureFile = new File(file.getPath().replace(ext, ".obj"));
			...
			try{
				ICachableImage fileImage = contentFactory.create(ICachableImage.class, file.getName(), UUID.randomUUID());
				fileImage.setImage(file);
				
				RotateTranslateScaleBehaviour rts = stage.getBehaviourMaker().addBehaviour(fileImage, RotateTranslateScaleBehaviour.class);
				...
				if (deceleration >= 0){
					behaviourMaker.addBehaviour(fileImage, NetworkFlickBehaviour.class).setDeceleration(deceleration);
				}
				...
				FeedbackSystem.registerAsFeedbackEligible(fileImage, fileImage.getWidth(), fileImage.getHeight(), stage);
			...
			try{
				...
				final IThreeDeeContent threeDItem = stage.getContentFactory().create(IThreeDeeContent.class, file.getName(), UUID.randomUUID());				
				threeDItem.setModel(file); 
				for (String ext: TEXTURE_EXTENSIONS){
					File textureFile = new File(file.getPath().replace(".obj", ext));
					if (textureFile.exists()){
				...
				threeDItem.setRelativeLocation(new Vector2f(-width/2, -height/2));
				...
				IContainer objectContainer = contentFactory.create(IContainer.class, file.getName() + "Container", UUID.randomUUID());
				
				IColourRectangle objectBackground = stage.getContentFactory().create(IColourRectangle.class, file.getName() + "Bg", UUID.randomUUID());
				...
				IImage listener = stage.getContentFactory().create(IImage.class, file.getName() + "listenBlock", UUID.randomUUID());
				listener.setSize(OBJ_SIZE - (BORDER_SIZE * 2), OBJ_SIZE - (BORDER_SIZE * 2));	
				
				IRoundedBorder objectBorder = stage.getContentFactory().create(IRoundedBorder.class, file.getName() + "Border", UUID.randomUUID());		
...
//				objectContainer.getZOrderManager().unregisterForZOrdering(objectBackground);
				...
				ThreeDRotateInteraction threeDRotateInteraction = stage.getBehaviourMaker().addBehaviour(listener, ThreeDRotateInteraction.class);
				threeDRotateInteraction.setItemActingOn(threeDItem);
				threeDRotateInteraction.setParentItem(objectContainer);
				
				ThreeDRotateInteraction threeDRotateInteractionObject = stage.getBehaviourMaker().addBehaviour(threeDItem, ThreeDRotateInteraction.class);
				...
				//This could be an issue in future circumstances - could make a cachable wrapper
				FeedbackSystem.registerAsFeedbackEligible(objectContainer, OBJ_SIZE, OBJ_SIZE, stage);  
			} catch (Exception e) {
				log(Level.SEVERE, "Exception", e);
			}
		}
	...
	* 
	* @param level
	*          The level indicating the severity of the log message.
	* @param logMessage
	...
	*          The image file the item is to display.
	* @param loc
	*          The location of the item on screen.
	~ @param stage
	*          Environment's stage.
	* @param rot
	*          Rotation of the item on screen.
	* @param displayWidth
	...
	* 
	**/
	public static void buildScreenshotItem(File screenShotFile, Vector2f loc, IStage stage, float rot){
		try{
			...
			ScreenshotContainer screenshotWrapperFrame = contentFactory.create(IScreenshotContainer.class, screenShotFile.getName(), UUID.randomUUID());
			screenshotWrapperFrame.setScreenShotImage(screenShotFile, stage, displayWidth/4, displayHeight/4);
					
			FeedbackSystem.registerAsFeedbackEligible(screenshotWrapperFrame, displayWidth/4, displayHeight/4, stage);
	...
	* 
	* @param loc
	*          The location to check is inside the box.
	* @param boxLoc
	...
	**/
	public static boolean inBox(Vector2f loc,  Vector2f boxLoc, Vector2f dim){
		if (loc.x > boxLoc.x - (dim.x/2) && loc.x < boxLoc.x + (dim.x/2) && loc.y > boxLoc.y - (dim.y/2) && loc.y < boxLoc.y + (dim.y/2))return true;
		return false;
	}	
	...
	* 
	* @param loc
	*          The location to check is inside the circle.
	* @param circleLoc
	...
	* 
	**/
	public static boolean inRadius(Vector2f loc,  Vector2f circleLoc, float radius){
		if (loc.distance(circleLoc) < radius)return true;
		return false;
	}	
