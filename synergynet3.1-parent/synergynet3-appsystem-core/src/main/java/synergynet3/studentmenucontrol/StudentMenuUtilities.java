...
public class StudentMenuUtilities {
	...
	* 
	**/
	public static StudentMenu login(String studentID, IStage stage, SynergyNetApp app) {
		
		for (StudentRepresentation studentRepresentation : studentRepresentations){
			if (studentRepresentation.getStudentId().equalsIgnoreCase(studentID))return null;
		}		
			
		int displayWidth = (int) (stage.getWorldLocation().x * 2);
		int displayHeight = (int) (stage.getWorldLocation().y * 2);
		
		StudentRepresentation studentRepresentation = DatabaseActivity.getStudentRepresentationFromDatabase(studentID, SynergyNetCluster.get().getXMPPConnection().getHost(), stage);
		StudentMenu menu = StudentMenuUtilities.generateStudentMenu(studentRepresentation, stage, app);
		
		menu.getRadialMenu().setRelativeLocation(new Vector2f((FastMath.rand.nextFloat()*(displayWidth-40)) - (displayWidth-40)/2,
				(FastMath.rand.nextFloat()*(displayHeight-40)) - (displayHeight-40)/2));
		menu.getRadialMenu().setRelativeRotation(FastMath.DEG_TO_RAD * (FastMath.rand.nextFloat()*360f));    
	...
	**/
	public static void logoutAll(IStage stage){
		for (int i = 0; i < studentRepresentations.size(); i++){
			try{
				DatabaseActivity.updateStudentRep(studentRepresentations.get(i), SynergyNetCluster.get().getXMPPConnection().getHost());
	...
	**/
	public static void logout(String studentID, IStage stage) {
		int toRemove = -1;
		for (int i = 0; i < studentRepresentations.size(); i++){
			if (studentID.equals(studentRepresentations.get(i).getStudent().getStudentID())){
				toRemove = i;
				break;
			}
		}
		...
		if (toRemove >= 0){		
			try{
				DatabaseActivity.updateStudentRep(studentRepresentations.get(toRemove), SynergyNetCluster.get().getXMPPConnection().getHost());
				studentRepresentations.get(toRemove).getGallery().removeFrom(stage);
	...
	*/
	public static void logoutAllOfClass(String className, IStage stage) {
		...
		for (int i = 0; i < studentRepresentations.size(); i++){
			try{
				if (studentRepresentations.get(i).getStudent().getClassName().equalsIgnoreCase(className)){
					DatabaseActivity.updateStudentRep(studentRepresentations.get(i), SynergyNetCluster.get().getXMPPConnection().getHost());
					studentRepresentations.get(i).getGallery().removeFrom(stage);
					studentMenus.get(i).removeMenu(stage);
