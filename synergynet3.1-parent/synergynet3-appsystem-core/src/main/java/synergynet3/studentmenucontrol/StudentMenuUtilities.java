...
public class StudentMenuUtilities
{
	...
	public static StudentMenu login(String studentID, IStage stage, SynergyNetApp app)
	{
		...
		{
			if (studentRepresentation.getStudentId().equalsIgnoreCase(studentID))
			{
				return null;
			}
		}
		...
		menu.getRadialMenu().setRelativeLocation(new Vector2f((FastMath.rand.nextFloat() * (displayWidth - 40)) - ((displayWidth - 40) / 2), (FastMath.rand.nextFloat() * (displayHeight - 40)) - ((displayHeight - 40) / 2)));
