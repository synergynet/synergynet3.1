 ...
 */
public class SNScreenshotAppState extends AbstractAppState implements
		SceneProcessor {
		...
		if (capture) {
			...
			try {
				SimpleDateFormat yearFormat = new SimpleDateFormat(
						"yyyyMMddHHmmssSS");
				String screenshotID = SynergyNetApp.getTableIdentity() + "_"
						+ yearFormat.format(new Date());
				File file = new File(CacheOrganisation.getScreenshotDir()
						+ File.separator + screenshotID + ".png");
				file.createNewFile();
				...
				AdditionalSynergyNetUtilities.log(Level.SEVERE,
						"Error while saving screenshot", ex);
			}
			loc = new Vector2f();
			rot = 0;
		}
	 ...
	 * Take screen shot.
	 *
	 * @param screenShotter the screen shotter
	 * @param loc the loc
	 * @param rot the rot
	 */
	public void takeScreenShot(IScreenShotter screenShotter, Vector2f loc,
			float rot) {
		this.loc = loc;
		this.rot = rot;
