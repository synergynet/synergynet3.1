...
public class ScreenshotUtilities {
	 ...
	 * from.
	 */
	public static void transferScreenShot(final File screenShotFile,
			final String[] projectorsToSendTo, final IStage stage) {
		Thread cachingThread = new Thread(new Runnable() {
			public void run() {
				try {
					...
					IScreenshotContainer item = contentFactory.create(
							IScreenshotContainer.class,
							screenShotFile.getName(), UUID.randomUUID());

					item.setScreenShotImage(screenShotFile, stage,
					...
					ProjectorTransferUtilities.get().addToTransferableContents(
							item, new Float(displayWidth / 4),
							new Float(displayHeight / 4),
							screenShotFile.getName());
					ProjectorTransferUtilities.get().transferIndividualItem(
							item, projectorsToSendTo,
							new Float(displayWidth / 4),
							new Float(displayHeight / 4));
					...
					AdditionalSynergyNetUtilities.log(Level.SEVERE,
							"Unable to transfer Screenshot", e);
				}
