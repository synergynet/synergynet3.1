...
public class CachableImage extends JMEImage implements ICachableImage,
	 ...
	 */
	public static CachableImage reconstruct(
			GalleryItemDatabaseFormat galleryItem, IStage stage, String loc) {
		try {
			CachableImage image = stage.getContentFactory().create(
					ICachableImage.class,
					(String) galleryItem.getValues().get(0), UUID.randomUUID());
			try {
				image.setImage(new File(CacheOrganisation.getSpecificDir(loc)
						+ File.separator
						+ (String) galleryItem.getValues().get(0)));
			} catch (Exception e) {
				image.setImage(CacheOrganisation.getSpecificDir(loc)
						+ File.separator
						+ (String) galleryItem.getValues().get(0));
			}
	...
	public void generateBorder(IStage stage, ColorRGBA borderColour,
			float borderWidth) {
		try {
			border = stage.getContentFactory().create(IRoundedBorder.class,
