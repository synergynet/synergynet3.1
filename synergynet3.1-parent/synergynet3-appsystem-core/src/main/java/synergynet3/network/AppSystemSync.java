...
public class AppSystemSync {
	...
	private DistributedPropertyChangedAction<String> loginAction = new DistributedPropertyChangedAction<String>() {
		...
		public void distributedPropertyDidChange(Member member,	String oldValue, String newValue) {
			if (!newValue.equals(""))snNode.login(newValue);	
		}
	};
	...
	private DistributedPropertyChangedAction<String> logoutAction = new DistributedPropertyChangedAction<String>() {
		@Override
		public void distributedPropertyDidChange(Member member,	String oldValue, String newValue) {
			if (!newValue.equals(""))snNode.logout(newValue);
		}
	};
	...
	/** Listener for network messages instigating the recreation of items from a device's directory in the networked shared cache. */
	private DistributedPropertyChangedAction<PerformActionMessage> reloadServerContentsAction = new DistributedPropertyChangedAction<PerformActionMessage>() {
		...
		public void distributedPropertyDidChange(Member member,	PerformActionMessage oldValue, PerformActionMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE)snNode.reloadServerCache();	
		}
	};
	
	/** Listener for network messages instigating the recreation of items from a removable drive. */
	private DistributedPropertyChangedAction<PerformActionMessage> reloadRemovableDriveContentsAction = new DistributedPropertyChangedAction<PerformActionMessage>() {
		@Override
		public void distributedPropertyDidChange(Member member,	PerformActionMessage oldValue, PerformActionMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE)snNode.reloadRemovableMediaCache();
		}
	};
	
	/** Listener for network messages instigating bringing student icons to the top of an environment. */
	private DistributedPropertyChangedAction<PerformActionMessage> bringStudentsToTopAction = new DistributedPropertyChangedAction<PerformActionMessage>() {
		@Override
		public void distributedPropertyDidChange(Member member,	PerformActionMessage oldValue, PerformActionMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE)snNode.bringAllStudentsToTop();			
		}
	};	
	
	/** Listener for network messages instigating the creation of a screenshot item. */
	private DistributedPropertyChangedAction<PerformActionMessage> takeScreenShot = new DistributedPropertyChangedAction<PerformActionMessage>() {
		@Override
		public void distributedPropertyDidChange(Member member,	PerformActionMessage oldValue, PerformActionMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE)snNode.createScreenShotItem(new Vector2f(), 0);			
		}
	};	
	
	/** Listener for network messages instigating the removal of any items in an environment not directly created by the application. */
	private DistributedPropertyChangedAction<PerformActionMessage> removeAdditionalMedia = new DistributedPropertyChangedAction<PerformActionMessage>() {
		@Override
		public void distributedPropertyDidChange(Member member,	PerformActionMessage oldValue, PerformActionMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE)snNode.removeAdditionalMedia();			
		}
	};	
	
	/** Listener for network messages announcing the location of another device's interface position. */
	...
	/** Listener for network messages announcing the arrival of an item transferred through a network flick. */
	private DistributedPropertyChangedAction<FlickMessage> networkflick = new DistributedPropertyChangedAction<FlickMessage>() {
		...
		public void distributedPropertyDidChange(Member member,	FlickMessage oldValue, FlickMessage newValue) {
			if (!newValue.messageAlreadyReceived() && newValue.getMessageState() == MESSAGESTATE.ACTIVATE){
				if (newValue.getTargetTableID() != null)snNode.onFlickArrival(newValue);			
			}
		}
	};	
	
	/** Listener for network messages announcing whether network flicks are enabled for this device. */
	...
	/** Listener for network messages initiating the transfer of a screenshot to projectors. */
	...
	/** Listener for network messages initiating the transfer of contents to projectors. */
	...
	private DistributedPropertyChangedAction<ArrayList<ContentTransferedMessage>> recieveContentsFromProjector = new DistributedPropertyChangedAction<ArrayList<ContentTransferedMessage>>() {
		...
		public void distributedPropertyDidChange(Member member,	ArrayList<ContentTransferedMessage> oldValue, ArrayList<ContentTransferedMessage> newValue) {
			if (newValue.size() > 0)snNode.onContentFromProjectorArrival(newValue);
		}
	};	
	...
	/**
	 * Initiates the listeners used for default SynergyNetApp extending application network cluster entries.
	 */
	...
	/**
	 * Stops the listeners used for default SynergyNetApp extending application network cluster entries.
	 */
	public void stop(){
