...
public class MultiTouchInputUtility {
	...
	private static final Logger log = Logger
			.getLogger(MultiTouchInputUtility.class.getName());
	...
	public static IMultiTouchInputSource getInputSource(
			InputManager inputManager, int displayWidth, int displayHeight) {
		...
		try {
			if (tabletype.equals(DisplayPrefsItem.INPUT_TYPES[0])) {
				multiTouchInput = new JMEDirectSimulator(inputManager,
				...
				multiTouchInput = new Win7TouchInput(tablePrefs.getWidth(),
						tablePrefs.getHeight(), false);
			} else if (tabletype.equals(DisplayPrefsItem.INPUT_TYPES[4])) {
				multiTouchInput = new Win7TouchInput(tablePrefs.getWidth(),
						tablePrefs.getHeight(), true);
			}
