...
public class KeyboardBehaviour implements IBehaviour, IMultiTouchEventListener {
	...
	public void cursorReleased(MultiTouchCursorEvent event) {
		...
		if ((kk != null) && kk.isEnabled()) {
			...
			if (keyPressTimes.containsKey(kk.getKeyStringRepresentation())) {
				long lastTime = keyPressTimes.get(kk
			...
			keyPressTimes.put(kk.getKeyStringRepresentation(),
	 ...
	 *
	 * @param milliseconds the new minimum time between key presses ms
	 */
	...
	private KeyboardKey getKeyUnderEvent(MultiTouchEvent event) {
		...
		Vector2f localPosition = item
				.getRelativeLocationOfWorldLocation(new Vector2f(
						screenPosition.x, screenPosition.y));
		localPosition.x = localPosition.x += (float) item
				.getKeyboardDefinition().getBounds().getWidth() / 2.0f;
		localPosition.y = localPosition.y += (float) item
				.getKeyboardDefinition().getBounds().getHeight() / 2.0f;
		Point2D p = new Point2D.Float(localPosition.x, (float) item
				.getKeyboardDefinition().getBounds().getMaxY()
				- localPosition.y);
