...
//			// rather than update the lines at the end of this update() method in an iteration
//			// through the Edge collection - doing the update now to save that pass through the
//			// edges. The change in location of the lines will probably be minimal anyway.
//
////			e.line.updateEndPoints();
//			
//			Vector2f v = e.b.item.getRelativeLocation().subtract(e.a.item.getRelativeLocation());
//			
//			float len = v.length();	if(len == 0) len = 0.0001f;
//
//			float forceBetweenNodes = (len - desiredLineLength) / (len * 3f);
//			v.multLocal(forceBetweenNodes * timePerFrameSeconds); // take into account time between successive frames
//
...
//			if(!n.shouldIgnore()) {
//				// as setRelativeLocation will trigger another method in Graph that inspects nodes.values, need to do this hack :-(
//				updateList.add(n);
//			}
//		}
//		
