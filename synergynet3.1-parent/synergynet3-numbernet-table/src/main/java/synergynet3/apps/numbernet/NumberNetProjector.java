...
public class NumberNetProjector implements IMultiplicityApp {
	...
	/** The Constant log. */
	private static final Logger log = Logger.getLogger(NumberNetProjector.class
			.getName());
	 ...
	 */
	@Override
	public void shouldStart(MultiTouchInputComponent input,
		...
		final ProjectorDevice device = new ProjectorDevice(SynergyNetCluster
				.get().getIdentity());
		...
		new ProjectionModeSynchronizer(device, projectScoresUI, scoresSync,
