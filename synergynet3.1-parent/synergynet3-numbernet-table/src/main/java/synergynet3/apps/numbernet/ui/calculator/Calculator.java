...
public class Calculator implements IMultiTouchKeyboardListener {
	...
	public void buildAndAttach() throws ContentTypeNotBoundException {
		...
		keyboardWrapperFrame = contentFactory.create(IContainer.class, "keywf",
				UUID.randomUUID());
		parent.addItem(keyboardWrapperFrame);

		keypadKeyboard = createKeypad(contentFactory);
		keyboardWrapperFrame.addItem(keypadKeyboard);
		Vector2f keypadSize = new Vector2f((float) keypadKeyboard
				.getKeyboardDefinition().getBounds().getWidth(),
				(float) keypadKeyboard.getKeyboardDefinition().getBounds()
						.getHeight());

		KeyboardBehaviour keyboardBehaviour = behaviourMaker.addBehaviour(
				keypadKeyboard, KeyboardBehaviour.class);
		keyboardBehaviour.addListener(this);

		IRoundedBorder keyboardWrapperFrameBorder = contentFactory.create(
		...
		keyboardWrapperFrameBorder.setColor(new ColorRGBA(borderGreyValue,
				borderGreyValue, borderGreyValue, borderTransparency));

		float outputLabelWidth = keypadSize.x + 60f;
		float outputLabelHeight = 60f;

		IRoundedBorder outputLabelBorder = contentFactory.create(
				IRoundedBorder.class, "border", UUID.randomUUID());
		...
		outputLabelBorder.setColor(new ColorRGBA(borderGreyValue,
				borderGreyValue, borderGreyValue, borderTransparency));

		outputLabelBackground = contentFactory.create(IColourRectangle.class,
				"labelbg", UUID.randomUUID());
		...
		Gradient greenGradient = new Gradient(topGreen, bottomGreen,
		...
		RotateTranslateScaleBehaviour rts = behaviourMaker
				.addBehaviour(keyboardWrapperFrameBorder,
	...
	public void keyReleased(KeyboardKey k, boolean shiftDown, boolean altDown,
		...
		if (k.getKeyCode() == KeyEvent.VK_C) {
			if (currentOutputValue.length() != 0) {
				setTextForCalculatorDisplay(currentOutputValue.substring(0,
						currentOutputValue.length() - 1));
	 ...
	 *
	 * @param result the new display style
	 */
	 ...
	 *
	 * @param keyStringRepresentation the key string representation
	 * @param state the state
	 */
	public void setKeyVisible(String keyStringRepresentation, boolean state) {
		this.keypadKeyboard.getKeyboardDefinition()
				.getKey(keyStringRepresentation).setEnabled(state);
	 ...
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private IMutableLabel createCalculatorOwnerLabel(IContentFactory cf)
			throws ContentTypeNotBoundException {
		IMutableLabel ownerLabel = cf.create(IMutableLabel.class, "",
				UUID.randomUUID());
		ownerLabel.setFont(FontUtil.getFont(FontColour.Blue));
		ownerLabel.setText(this.owner);
		ownerLabel.setBoxSize((float) keypadKeyboard.getKeyboardDefinition()
				.getBounds().getWidth(), ownerLabel.getTextSize().y);
	 ...
	 *
	 * @param cf the cf
	 * @param outputLabelWidth the output label width
	 * @return the i mutable label
	 * @throws ContentTypeNotBoundException the content type not bound exception
	 */
	private IMutableLabel createKeypadOutputLabel(IContentFactory cf,
			float outputLabelWidth) throws ContentTypeNotBoundException {
		IMutableLabel outputLabel = cf.create(IMutableLabel.class, "",
				UUID.randomUUID());
		outputLabel.setFont(FontUtil.getFont(FontColour.White));
		outputLabel.setText("");
		outputLabel.setBoxSize(outputLabelWidth / 1.6f,
				outputLabel.getTextSize().y);
		return outputLabel;
	}
