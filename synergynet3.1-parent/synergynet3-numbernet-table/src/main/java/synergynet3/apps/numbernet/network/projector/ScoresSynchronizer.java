...
public class ScoresSynchronizer
{
	...
	private int getBracketUseCount(Double target, String table)
	{
		...
		{
			if (e.isCorrect() && e.getCreatedOnTable().equals(table) && (e.getExpression().contains("(") && e.getExpression().contains(")")))
			{
				count++;
			}
		}
	...
	private void registerAsListenerForChangesInDistributedMap(final Double target, DistributedMap<String, Expression> map)
	{
		...
		{
			@Override
			public void itemUpdatedInCollection(IMap<String, Expression> collection, String itemKey, Expression itemOldValue, Expression itemNewValue, Member member)
			{
				mapChangedForTarget(target);
			}
		});
