...
public class FlickGameApp extends SynergyNetApp implements IMultiTouchEventListener {	
	...
	private void enableCollisionDetection(final IItem item){
		CollisionBoxAnimationElement collisionAnimationElement = new CollisionBoxAnimationElement(item){
			...
			protected void onEnteringCollision(IItem collidale){
				...
				try {
					if (blueScore < scoreLimit && redScore < scoreLimit)generateBall();
				} catch (ContentTypeNotBoundException e) {
					AdditionalSynergyNetUtilities.log(Level.SEVERE, "Content Type Not Bound", e);
				}
	...
	private void showStartScreen(boolean isVisible){
		for(IItem item: startScreenItems)item.setVisible(isVisible);
		for(IItem item: gameItems)item.setVisible(!isVisible);
		...
		if (!isVisible){
			...
			writeToLog();
			destroyPaddles();
			if (paddleChoice)paddleNumber = 1;
			updatePaddlesLabel();
	...
	private void tidyAwayIrrelevantItems() {
		...
		for (IItem item: stage.getChildItems()){
			...
			if (item.equals(scoreLabelBlue) || item.equals(scoreLabelRed))shouldRemove = false;
			if (shouldRemove){
				...
				if (shouldRemove){
					...
					if (shouldRemove)toRemove.add(item);	
				}		
			}
		}
		for (IItem item: toRemove)stage.removeItem(item);
	}
	...
	private void generatePaddleNumberControls() throws ContentTypeNotBoundException {
		...
		decreasePaddlesButton.getListener().getMultiTouchDispatcher().addListener(new MultiTouchEventAdapter() {
			@Override
			public void cursorClicked(MultiTouchCursorEvent event) {					
				if (paddleNumber > 1)paddleNumber--;
				updatePaddlesLabel();
			}
		});	
		...
		increasePaddlesButton.getListener().getMultiTouchDispatcher().addListener(new MultiTouchEventAdapter() {
			@Override
			public void cursorClicked(MultiTouchCursorEvent event) {					
				if (paddleNumber < PADDLE_LIMIT)paddleNumber++;
				updatePaddlesLabel();
			}
		});
	...
	private void generatePaddles() throws ContentTypeNotBoundException{
		...
		if (tableColour.equals("red"))fileName = "red_paddle.png";
	...
	public void updateScore(FlickGameScore score) {
		...
		if (blueScore == 0 && redScore == 0 && initialised)showStartScreen(false);
		setScoreLabel();
	}
	
	private void setScoreLabel(){
		if (scoreLabelBlue != null)scoreLabelBlue.setText(""+blueScore);
		if (scoreLabelRed != null)scoreLabelRed.setText(""+redScore);
		if (blueScore >= scoreLimit || redScore >= scoreLimit)showStartScreen(true);		
	}
	...
	public void onDestroy() {
		if (flickGameSync != null)flickGameSync.stop();
		super.onDestroy();
	}
	...
	public static void main(String[] args) {
		...
		if (tableColour == null)tableColour = "red";
