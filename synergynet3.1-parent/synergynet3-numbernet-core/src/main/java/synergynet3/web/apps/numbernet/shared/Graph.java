...
public class Graph {
	 ...
	 *         edges
	 */
	public List<Edge> getEdgesLongerThanDistanceIgnoringCurrentlyInteractingEdges(
			float distance) {
		List<Edge> edgesLongerThanDistance = new ArrayList<Edge>();
		for (Edge e : edges) {
			if ((e.getLength() >= distance) && e.a.shouldIgnore()
					&& e.b.shouldIgnore()) {
	...
	public boolean isConnected(String a, String b) {
		for (Edge e : edges) {
			if ((e.a.getID().equals(a) && e.b.getID().equals(b))
					|| (e.a.getID().equals(b) && e.b.getID().equals(a))) {
				return true;
			}
		}
		return false;
