...
public class JME3ContentSystemFactory extends DynamicContentSystemFactory {
	...
	private static final Logger log = Logger
			.getLogger(JME3ContentSystemFactory.class.getName());
	 ...
	 * @param audioRenderer the audio renderer
	 ...
	 */
	public JME3ContentSystemFactory(Renderer renderer,
			AudioRenderer audioRenderer, AssetManager assetManager,
			List<IUpdateable> updateList) throws ClassNotFoundException,
		...
		super("multiplicity3.jme3csys.items", new IResourceFinderFilter() {
			...
			public boolean accept(String dottedResourcePathname) {
				...
				return extension.equals("class") && name.startsWith("JME")
						&& !name.endsWith("Delegate") && !name.endsWith("$1");
			}
		});
	...
	public <ContentType extends IItem, ConcreteType extends IItem> ConcreteType create(
		...
		log.fine("Instance of " + clazz.getName() + " created: "
	 ...
	 *
	 * @param audioRenderer the new audio renderer
	 */
