...
//		redoing = false;
					...
					public void cursorChanged(MultiTouchCursorEvent event) {
						if (angles.containsKey(event.getCursorID())){
							...
							float angle = new Vector2f(touchDrag.x - origin.x, touchDrag.y - origin.y).angleBetween(
									new Vector2f(touchDowns.get(event.getCursorID()).x - origin.x, touchDowns.get(event.getCursorID()).y - origin.y));
	...
	private void okOnArrows(){
		...
		if (!TablePosUtilities.isWithinXDegreesOf(FastMath.DEG_TO_RAD * 100, actualTableO, tableOArray[currentLoop])){						
			setupInitialContent();
			redoLog.add("Execution " + (currentLoop+1) + "," + (FastMath.RAD_TO_DEG * tableOArray[currentLoop]) + ", " + (FastMath.RAD_TO_DEG * lineToAAngleArray[currentLoop]) 
					+ ", " + (FastMath.RAD_TO_DEG * lineToBAngleArray[currentLoop]) + "User's orientation is over 100 degrees from the table's actual orientation.");	
			return;
		}
...
//				if (!(isAngleWithinStandardDeviation(tableOArray[i], orientationAVG, orientationSD) && isAngleWithinStandardDeviation(lineToAAngleArray[i], aAVG, aSD)
//						&& isAngleWithinStandardDeviation(lineToBAngleArray[i], bAVG, bSD))){
//					redoing = true;
//					toRedo.add(i);								
//					redoLog.add("Execution " + (i+1) + " is outside the standard deviation, " + (FastMath.RAD_TO_DEG * tableOArray[i])
//							+ ", " + (FastMath.RAD_TO_DEG * lineToAAngleArray[i]) + ", " + (FastMath.RAD_TO_DEG * lineToBAngleArray[i]));							
//				}
//			}
//			
//			if (redoing){
...
//			}
	...
	private void recordLineAngles(){
		...
		if (SArray[currentLoop] < 0)TArray[currentLoop] += (FastMath.DEG_TO_RAD * 380);		
	...
	private void calculateTableInfo(){		
		...
		if (S < 0)T += (FastMath.DEG_TO_RAD * 380);		
		...
		TablePosUtilities.writeToLog(logAddress, startTime, lineToBAngle, lineToAAngle, S, T, R, A, B, d, actualLineToAAngle, actualLineToBAngle, actualS, actualT, actualR, actualA, actualB, actuald, 
				actualTableX, actualTableY, actualTableO, referenceDistance, loops, firstTouch, changeOver, lastTouch, orientationTouchCount, positioningTouchCount, lineToAAngleArray, lineToBAngleArray, 
				SArray, TArray, RArray, AArray, BArray, tableOArray, XArray, YArray, dArray, tableX, tableY, tableO, redoLog);
	}
...
//
//		float standardDeviation =  ((FastMath.atan2(sinSD, cosSD)) + (FastMath.DEG_TO_RAD * 360)) % (FastMath.DEG_TO_RAD * 360);
//		
//		return standardDeviation;
//	}	
//	
//	private float getStandardDeviationOfLinearValues(float[] values, float avg){		
//		float sd = 0;
//		for (int i=0; i< values.length; i++)sd += FastMath.sqr(standardiseAngle(values[i])-avg);
//		sd /= values.length;
//		sd = FastMath.sqrt(sd);
//		return sd;
//	}	
//	
//	private boolean isAngleWithinStandardDeviation(float value, float average, float standardDeviation) {		
