...
public class Universe extends AnimationElement {
	...
	private static final Logger log = Logger
			.getLogger(Universe.class.getName());
	 ...
	 * Sets the scenario.
	 *
	 * @param newValue the new scenario
	 */
	public void setScenario(UniverseScenario newValue) {
		...
		switch (newValue) {
			case SUN_AND_MOONS: {
				...
				Body sun = new Body("sun", new MassReference(MASS_SUN),
						new Vector2f(512, 768 / 2), new Vector2f(0, 0));
				log.fine("adding sun");
				addBody(sun);
			...
			case BINARY_STAR_SYSTEM: {
				...
				vel1.multLocal(5e3f);
				Body sun = new Body("sun", new MassReference(MASS_SUN),
						middle.add(new Vector2f(0, distDiff)), vel1);
				log.fine("adding sun");
				addBody(sun);
				log.fine("updating position via delegate");
				delegate.bodyPositionChanged(sun);
				...
				vel2.multLocal(5e3f);
				Body sun2 = new Body("sun", new MassReference(MASS_SUN),
						middle.add(new Vector2f(0, -distDiff)), vel2);
				log.fine("adding sun");
				addBody(sun2);
				log.fine("updating position via delegate");
