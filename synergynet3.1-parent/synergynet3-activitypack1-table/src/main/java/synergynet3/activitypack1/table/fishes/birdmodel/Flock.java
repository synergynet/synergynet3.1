/**
 * File Name: Flock.cs Date: 06/01/2006 Copyright (c) 2006 Michael LaLena. All
...
public class Flock {
	...
	synchronized public Vector<Bird> move() {
		...
		while (movingBird < birds.size()) {
			...
			Bird bird = birds.birdAtIndex(movingBird);
			...
			while (birdToEat < birds.size()) {
				Bird otherBird = birds.birdAtIndex(birdToEat);
				...
				if ((movingBird != birdToEat)
					...
					// If this bird is a predator and the other Bird is a normal
					// bird
					// or is the other bird food
					if (((bird instanceof Predator) && (otherBird.getClass()
							.equals(Bird.class)))
							|| (otherBird instanceof Food)) {
			...
			boolean ateBirdBeforeCurrent = (ateBird)
					&& (birdToEat < movingBird);
	...
	public Vector2f sumPoints(Vector2f p1, double w1, Vector2f p2, double w2) {
		return new Vector2f((float) ((w1 * p1.x) + (w2 * p2.x)),
				(float) ((w1 * p1.y) + (w2 * p2.y)));
	}
	...
	private float generalHeading(Bird bird) {
		...
		for (int i = 0; i < birds.size(); i++) {
			...
			Vector2f otherLocation = closestLocation(bird.getLocation(),
					otherBird.getLocation());
			...
			if (!bird.equals(otherBird) && (distance > 0)
				...
				if (bird.getColor().equals(otherBird.getColor())) { // birds of
																	...
																	// color
																	// attract
					Vector2f align = new Vector2f(
							(float) (100 * Math
									.cos((otherBird.getTheta() * Math.PI) / 180)),
					...
					if (tooClose) {
						weight *= Math.pow(
								1 - ((double) distance / SeparationRange), 2);
					} else {
						weight *= -Math
								.pow((double) (distance - SeparationRange)
										/ (DetectionRange - SeparationRange), 2);
					}
					Vector2f attract = sumPoints(otherLocation, -1.0,
							bird.getLocation(), 1.0);
				...
				/*
				 * If this bird is a predator, and the other bird is a normal
				 * bird then there is again attraction, but the weight is fixed
				 * at 1.
				 */
				else if ((bird instanceof Predator)
						&& (otherBird.getClass().equals(Bird.class))) {
					Vector2f dist = sumPoints(bird.getLocation(), -1.0,
							otherLocation, 1.0);
					dist = normalisePoint(dist, 1000);
					double weight = 1;
					target = sumPoints(target, 1.0, dist, weight); // weight is
																	// variable
				}
				/*
				...
				/*
				 * If the birds are a different color (or if the other bird is
				 * actually an obstacle, the bird is repulsed with a force that
				 * is weighted according to a distance square rule.
				 */
				else {
					Vector2f dist = sumPoints(bird.getLocation(), 1.0,
							otherLocation, -1.0);
					...
					double weight = Math.pow(
							(1 - ((double) distance / DetectionRange)), 2);
					target = sumPoints(target, 1.0, dist, weight); // weight is
																	// variable
				}
				numBirds++;
		...
		if (numBirds == 0) {
			...
			target = sumPoints(bird.getLocation(), 1.0, target,
					1 / (double) numBirds);
		}

		// Turn the target location into a direction in degrees
		float targetTheta = (float) ((180 / Math.PI) * Math.atan2(
				bird.getLocation().y - target.y, target.x
						- bird.getLocation().x));
