	...
	synchronized public Vector<Bird> move()
	{
		...
		while (movingBird < birds.size())
		{
			...
			while (birdToEat < birds.size())
			{
				...
				// bird)
				if ((movingBird != birdToEat) && (bird.getLocation().x > (otherBird.getLocation().x - 2)) && (bird.getLocation().x < (otherBird.getLocation().x + 2)) && (bird.getLocation().y > (otherBird.getLocation().y - 2)) && (bird.getLocation().y < (otherBird.getLocation().y + 2)))
				{
		...
		for (int i = 0; i < birds.size(); i++)
		{
			...
			if (!bird.equals(otherBird) && (distance > 0) && (distance <= DetectionRange))
			{
				...
				{ // birds of
					...
					// color
					// attract
					Vector2f align = new Vector2f((float) (100 * Math.cos((otherBird.getTheta() * Math.PI) / 180)), (float) (-100 * Math.sin((otherBird.getTheta() * Math.PI) / 180)));
